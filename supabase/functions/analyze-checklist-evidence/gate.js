...
export const NOT_OBSERVABLE_MESSAGE = "Esta foto não é capaz de comprovar o que foi pedido. É necessária uma conferência de outra forma.";
const LOW_CONFIDENCE_MESSAGE = "Não deu para confirmar com segurança. Tire outra foto mais próxima e bem iluminada.";
export function conditionStatusOf(judge) {
    const raw = String(judge?.condition_status ?? "").trim();
    if (raw === "verified" || raw === "not_met" || raw === "not_observable")
        return raw;
...
export function judgeJsonComplete(judge) {
    if (!judge || typeof judge !== "object")
        return false;
    if (typeof judge.decision !== "string")
        return false;
 ...
 */
export function normalizeVerifiability(parsed) {
    ...
    let level = raw === "verifiable" || raw === "partially_verifiable" || raw === "not_verifiable"
        ? raw
        : "partially_verifiable";
    if (unverifiable.length > 0 && level === "verifiable")
        level = "partially_verifiable";
    if (conditions.length > 0 && unverifiable.length >= conditions.length)
        level = "not_verifiable";
    let count = Number(parsed?.required_evidence_count);
    if (!Number.isFinite(count) || count < 1)
        count = 1;
    count = Math.min(Math.round(count), 4);
    if (level !== "verifiable" && count < 2)
        count = 2;
    return {
...
export function canRelease(verifiability) {
    return verifiability === "verifiable";
}
export function combine(observer, judge, options = {}) {
    ...
    const confidence = typeof judge?.confidence === "number" && Number.isFinite(judge.confidence)
        ? judge.confidence
        : null;
    const base = { condition_status: null, confidence, confidence_threshold: threshold };
...
export function technicalFailure(reasonCode, threshold = DEFAULT_CONFIDENCE_THRESHOLD) {
    return {
