-- ============================================================================
-- B2 ajustes + B3: mínimos configuráveis, gestão de token, aprovação/ativação
-- ============================================================================

-- 1) Mínimos configuráveis por padrão visual --------------------------------
ALTER TABLE public.vision_datasets
  ADD COLUMN IF NOT EXISTS min_normal_technical    integer NOT NULL DEFAULT 5,
  ADD COLUMN IF NOT EXISTS min_normal_recommended  integer NOT NULL DEFAULT 20,
  ADD COLUMN IF NOT EXISTS min_anomalous_recommended integer NOT NULL DEFAULT 3;

-- 2) Versões: run selecionado + exceção de aprovação sem anomalias ----------
ALTER TABLE public.vision_model_versions
  ADD COLUMN IF NOT EXISTS selected_run_id uuid REFERENCES public.vision_model_runs(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS approval_exception_reason text,
  ADD COLUMN IF NOT EXISTS dispatched_to_service_at timestamptz;

-- 3) Auditoria de eventos de versão -----------------------------------------
CREATE TABLE IF NOT EXISTS public.vision_model_audit (
...
GRANT SELECT ON public.vision_model_audit TO authenticated;
GRANT ALL ON public.vision_model_audit TO service_role;
ALTER TABLE public.vision_model_audit ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "vision_model_audit_admin_read" ON public.vision_model_audit;
CREATE POLICY "vision_model_audit_admin_read"
  ON public.vision_model_audit
  FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'franqueadora'));

-- 4) prepare_model_version: usa mínimos configuráveis; permite 0 anomalias --
...
-- 7) Ativar versão (aprovação + aposentadoria atômicas) ---------------------
...
BEGIN
  ...
  IF v_ver.status <> 'awaiting_approval' THEN
    RAISE EXCEPTION 'version_not_awaiting_approval' USING ERRCODE='check_violation',
      MESSAGE=format('Versão em status %s; esperado awaiting_approval.', v_ver.status);
  END IF;
  ...
  END IF;
  SELECT COALESCE(jsonb_object_keys_count(v_run.metrics), 0) INTO v_metrics_keys FROM (
    SELECT COUNT(*) AS jsonb_object_keys_count FROM jsonb_object_keys(COALESCE(v_run.metrics,'{}'::jsonb))
  ) t;
  ...
  IF NOT v_has_anom THEN
    IF p_exception_reason IS NULL OR length(btrim(p_exception_reason))<10 THEN
      RAISE EXCEPTION 'anomaly_exception_required' USING ERRCODE='check_violation',
        MESSAGE='Ativação sem imagens anômalas exige motivo administrativo explícito (mín. 10 caracteres).';
    END IF;
  END IF;

  -- Aposenta ativo anterior deste padrão/workspace na mesma transação
  ...
  -- Marca versões anteriores do mesmo dataset em awaiting_approval como retiradas
  UPDATE public.vision_model_versions
     SET status='retired', retired_at=now(), updated_at=now()
   WHERE dataset_id=v_ver.dataset_id AND id<>p_version_id AND status='active';
...
-- Grants para RPCs
REVOKE ALL ON FUNCTION public.rotate_model_version_run_token(uuid,text) FROM public;
REVOKE ALL ON FUNCTION public.revoke_model_version_run_token(uuid,text) FROM public;
REVOKE ALL ON FUNCTION public.reject_model_version(uuid,text) FROM public;
REVOKE ALL ON FUNCTION public.activate_model_version(uuid,uuid,text) FROM public;
GRANT EXECUTE ON FUNCTION public.rotate_model_version_run_token(uuid,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.revoke_model_version_run_token(uuid,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.reject_model_version(uuid,text) TO authenticated;
