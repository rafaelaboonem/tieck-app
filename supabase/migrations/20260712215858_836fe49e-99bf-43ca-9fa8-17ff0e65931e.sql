...
BEGIN
  ...
  gs AS (
    SELECT group_key,
      CASE
        WHEN has_anom THEN
          -- Grupos com anomalia: metade validação, metade teste (nunca em treino).
          CASE WHEN rn <= GREATEST(1, (cnt+1)/2) THEN 'validation' ELSE 'test' END
        ELSE
          -- Grupos normais: garante pelo menos 1 em treino ordenando por hash determinístico.
          -- Alvo: 70% treino, 15% validação, 15% teste; treino tem prioridade quando cnt é pequeno.
          CASE
            WHEN rn <= GREATEST(1, floor(cnt * 0.70)::int) THEN 'train'
            WHEN rn <= GREATEST(1, floor(cnt * 0.70)::int) + GREATEST(0, floor(cnt * 0.15)::int) THEN 'validation'
            ELSE 'test'
          END
      END AS split
    FROM ranked
  ...
  IF v_tn < GREATEST(1, COALESCE(v_ds.min_normal_technical,5)) THEN
    RAISE EXCEPTION 'insufficient_training_images'
      USING DETAIL = format('Mínimo técnico do padrão exige %s imagem(ns) corretas em treino; obtidas %s.',
