CREATE OR REPLACE FUNCTION public.prepare_model_version(p_dataset_id uuid, p_note text DEFAULT NULL::text, p_seed bigint DEFAULT NULL::bigint)
 RETURNS TABLE(version_id uuid, snapshot_id uuid, version text, run_token text, run_token_expires_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_min_normal integer;
  v_normal_groups integer;
  v_token       text;
  v_token_hash  text;
...
BEGIN
  ...
  v_min_normal := GREATEST(1, COALESCE(v_ds.min_normal_technical, 5));
  ...
  -- Regra do mínimo técnico: contar imagens corretas ÚNICAS por SHA-256,
  -- independentemente de quantos envios (response_id) originaram as fotos.
  SELECT COUNT(*) INTO v_normal
  FROM (
    SELECT DISTINCT sha256
    FROM public.vision_curated_images
    WHERE dataset_id=p_dataset_id AND classification='normal' AND sha256 IS NOT NULL
  ) s;

  IF v_normal < v_min_normal THEN
    RAISE EXCEPTION 'insufficient_training_images'
      USING DETAIL = format('Mínimo técnico do padrão exige %s imagem(ns) corretas únicas; obtidas %s.',
                             v_min_normal, v_normal),
  ...
  -- Divisão determinística: mantém imagens do mesmo response no mesmo split,
  -- e GARANTE pelo menos 1 grupo normal em treino (mesmo com poucos grupos).
  WITH src AS (
  ...
  gf AS (
    SELECT group_key,
           ...
           (('x'||substr(md5(v_seed::text||':'||group_key),1,8))::bit(32)::int & 2147483647) AS h
    FROM src GROUP BY group_key
  ),
  ranked AS (
    SELECT group_key, has_anom,
           ROW_NUMBER() OVER (PARTITION BY has_anom ORDER BY h, group_key) AS rn,
           COUNT(*)     OVER (PARTITION BY has_anom)                       AS cnt
    FROM gf
  ),
  gs AS (
    SELECT group_key,
      CASE
        WHEN has_anom THEN
          CASE WHEN rn <= GREATEST(1, (cnt+1)/2) THEN 'validation' ELSE 'test' END
        ELSE
          CASE
            -- Prioridade absoluta: ao menos 1 grupo normal em treino.
            WHEN cnt <= 2 THEN 'train'
            -- 70% treino / 15% validação / 15% teste, com mínimos garantidos.
            WHEN rn <= GREATEST(1, floor(cnt * 0.70)::int) THEN 'train'
            WHEN rn <= GREATEST(1, floor(cnt * 0.70)::int) + GREATEST(1, floor(cnt * 0.15)::int) THEN 'validation'
            ELSE 'test'
          END
      END AS split
    FROM ranked
  ...
  IF v_tn < 1 THEN
    RAISE EXCEPTION 'insufficient_training_images'
      USING DETAIL = 'Nenhuma imagem correta foi alocada ao split de treino.',
            ERRCODE = 'check_violation';
  END IF;

  -- Contagem de grupos independentes normais (para aviso de baixa confiabilidade).
  SELECT COUNT(DISTINCT COALESCE(response_id::text,'img:'||id::text)) INTO v_normal_groups
  FROM public.vision_curated_images
  WHERE dataset_id=p_dataset_id AND classification='normal' AND sha256 IS NOT NULL;
  ...
  INSERT INTO public.vision_model_versions(
    ...
    'Preparando imagens',
    CASE WHEN v_normal_groups < 8
      THEN 'Snapshot criado. Poucos envios independentes podem reduzir a confiabilidade da validação.'
      ELSE 'Snapshot do dataset criado; aguardando execução do treino.'
    END,
  ...
  VALUES (v_version_id, v_caller, 'prepared',
    jsonb_build_object('snapshot_id', v_snapshot_id, 'seed', v_seed,
                       ...
                       'normal_groups', v_normal_groups,
                       'low_confidence_validation', v_normal_groups < 8));
