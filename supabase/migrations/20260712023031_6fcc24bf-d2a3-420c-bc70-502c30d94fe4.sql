...
BEGIN
  -- Só valida quando o checklist está sendo publicado (ou re-publicado).
  ...
  -- Organização do checklist via unit_id (quando existir).
  SELECT u.organization_id INTO cl_org
    FROM public.units u
  ...
  LOOP
    ...
    -- modelId presente e UUID válido.
    BEGIN
    ...
    -- Threshold no bloco: 0..1 (quando informado).
    ...
    -- Localiza o modelo e valida escopo/status/versão/artefato/provider.
...
DROP TRIGGER IF EXISTS trg_validate_checklist_publication ON public.checklists;
CREATE TRIGGER trg_validate_checklist_publication
  BEFORE INSERT OR UPDATE OF is_published, published_content
  ON public.checklists
  FOR EACH ROW
  WHEN (NEW.is_published = true)
  EXECUTE FUNCTION public.validate_checklist_publication();
