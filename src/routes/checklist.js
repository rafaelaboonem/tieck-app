...
import { Star, Camera, Trash2, Upload, ChevronDown, Sparkles, Loader2, AlertCircle, Info, Plus, Minus, Menu, CircleCheck, CheckSquare, ListChecks, Hash, Mail, Phone, Link2, Calendar, Clock, SlidersHorizontal, Grid3x3, PenLine, ArrowUpDown, ListTodo, PlusSquare, DollarSign, FilePlus, Type, Heading1, Heading2, Heading3, Bookmark, Tag, Image as ImageIcon, Video, Mic, QrCode, Code, Settings, X, EyeOff, Search, Palette, ArrowRight, Check, Copy, History, CheckCheck, Ban, GripVertical, Move, Bold, Italic, Underline, Highlighter, Hexagon, ArrowUpRight, ArrowRightCircle, ArrowBigRight, MoveRight, Send, SendHorizontal, CheckCircle, ChevronUp, ChevronLeft, Zap, FileText, LayoutTemplate, HelpCircle, Navigation, Network, ExternalLink } from "lucide-react";
...
import { ensureCameraBlockIds, withNewCameraBlockId } from "@/lib/camera-blocks";
import { hashQuestion } from "@/lib/camera-ai/hashing";
function CameraBlockPreview({ textColor, blockId }) {
    ...
    return (<div className="space-y-4">
      <div className={`w-full h-64 border-2 ${dataUrl ? 'border-solid' : 'border-dashed border-neutral-300'} dark:border-neutral-700 rounded-xl bg-white dark:bg-neutral-900/50 flex flex-col items-center justify-center gap-4 group transition-colors overflow-hidden relative`} style={dataUrl ? { borderColor: textColor } : undefined}>
        {dataUrl ? (<>
            <img src={dataUrl} alt="Capturada" className="w-full h-full object-cover animate-in fade-in zoom-in-95 duration-300 absolute inset-0 z-0"/>
            ...
            <button type="button" onClick={() => setDataUrl(null)} className="absolute top-4 right-4 z-10 bg-white/95 hover:bg-white text-neutral-700 text-xs font-medium px-3 py-1.5 rounded-md border border-neutral-200 shadow-sm">
              Tirar outra
            </button>
          </>) : isLive ? (<>
            <video ref={videoRef} playsInline muted className="absolute inset-0 w-full h-full object-cover z-0"/>
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 flex gap-2">
              <button type="button" onClick={capturePhoto} className="text-white text-sm font-semibold px-4 py-2 rounded-md shadow-lg hover:opacity-90 transition-opacity" style={{ backgroundColor: textColor }}>
                Capturar
              </button>
              <button type="button" onClick={stopStream} className="bg-white/95 hover:bg-white text-neutral-700 text-sm font-medium px-4 py-2 rounded-md border border-neutral-200 shadow">
                Cancelar
              </button>
            </div>
        ...
        <input ref={inputRef} id={`camera-input-${blockId}`} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => {
            const file = e.target.files?.[0];
            if (!file)
                return;
            const reader = new FileReader();
            reader.onload = (ev) => setDataUrl(ev.target?.result ?? null);
...
function CameraBlockEditor({ block, isActive, currentChecklistId, updateBlock, removeBlock, setActiveBlockId, textColor, textareaRefs }) {
    ...
    const triggerCompile = async (checklistId, blockId) => {
        if (isCompiling)
            return;
        setIsCompiling(true);
        try {
    ...
    return (<div ref={(el) => { if (el)
        textareaRefs.current[block.id] = el; }} tabIndex={0} onKeyDown={(e) => {
            if (e.key === "Backspace") {
      ...
      <CameraBlockCard block={block} isActive={isActive} isCompiling={isCompiling} textColor={textColor} onSelect={() => setActiveBlockId(block.id)}/>

      <CameraSettingsPanel block={block} isOpen={isActive} onClose={() => setActiveBlockId(null)} isCompiling={isCompiling} onSave={(patch) => updateBlock(block.id, patch)} checklistId={currentChecklistId || ""}/>
    </div>);
}
function ChecklistPreview({ blocks, title, onClose, onPublish, isPublishing, settings, }) {
    const { sidebarOpen } = useSidebar();
    const isDark = settings.theme === "Escuro";
    const [submitted, setSubmitted] = useState(false);
    return (<div className="fixed inset-0 z-[100] overflow-y-auto" style={{
      ...
      <div className="sticky top-0 z-[110] flex items-center justify-between px-6 py-3 bg-white/80 backdrop-blur-md border-b border-neutral-100 dark:bg-neutral-900/80 dark:border-neutral-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onPublish} disabled={isPublishing} className="text-xs font-bold bg-[#FF007F] text-white rounded-md px-4 py-1.5 hover:opacity-90 transition-opacity disabled:opacity-50 shadow-sm">
            {isPublishing ? "Publicando..." : "Publicar"}
          </button>
        </div>
      </div>

      {submitted ? (<main className="max-w-4xl mx-auto px-6 pt-32 pb-32 flex flex-col items-center text-center">
          ...
          <button type="button" onClick={() => setSubmitted(false)} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-blue-600 font-semibold hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors shadow-sm">
            <span className="text-pink-500">*</span> Crie seu próprio checklist
          </button>
        </main>) : (<main className="max-w-full mx-auto pb-32 relative">
        <div className="flex flex-col items-center w-full">
          <div className="w-full relative">
            <div className="absolute top-4 left-6 z-20">
              <button type="button" onClick={onClose} className="flex items-center gap-2 text-sm font-semibold px-4 py-2 bg-white/90 backdrop-blur-sm dark:bg-neutral-800/90 text-neutral-700 dark:text-neutral-200 border border-neutral-200 dark:border-neutral-700 rounded-lg hover:bg-white dark:hover:bg-neutral-800 transition-all shadow-md">
            ...
            {(() => {
                ...
                return (<div className={`w-full ${profileBlock?.src && coverBlock?.src ? "mb-20" : "mb-10"}`}>
                   <div className="relative">
                      {coverBlock?.src && (<div className="relative w-full h-80 overflow-hidden bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center">
                          <img src={coverBlock.src} alt="Cover" className="w-full h-full" style={{
                            transform: `translate(${coverBlock.position?.x || 0}px, ${coverBlock.position?.y || 0}px) scale(${coverBlock.position?.zoom || 1})`,
                     ...
                     {profileBlock?.src && (<div className={`${coverBlock?.src ? "absolute left-1/2 bottom-0 -translate-x-1/2 translate-y-1/2" : "flex justify-center mb-8"} z-10`}>
                          <div className="border-4 border-white dark:border-neutral-900 shadow-lg overflow-hidden bg-white" style={{
                            width: settings.logoWidth,
                            height: settings.logoHeight,
                            borderRadius: settings.logoRadius
                        }}>
                           <img src={profileBlock.src} alt="Profile" className="w-full h-full" style={{
                            transform: `translate(${profileBlock.position?.x || 0}px, ${profileBlock.position?.y || 0}px) scale(${profileBlock.position?.zoom || 1})`,
                            transformOrigin: "center center",
                            objectFit: "contain",
            ...
            {title && (<h1 className="text-4xl font-bold mb-10 tracking-tight" style={{ color: settings.textColor }}>{title}</h1>)}

            <div className="space-y-8 pb-[300px]">
              {blocks.map((block) => (<BlockRenderer key={block.id} block={block} settings={settings} mode="preview"/>))}

              {blocks.some(b => INTERACTIVE_BLOCK_TYPES.includes(b.type)) && (<div className="mt-12 mb-4 flex">
                  <button type="button" onClick={() => setSubmitted(true)} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold shadow-lg hover:opacity-90 transition-all active:scale-95" style={{
...
export function renderBtnIcon(key) {
...
export const Route = createFileRoute("/checklist")({
    ...
    component: () => (<ChecklistAuthGuard>
      <NovoChecklistPage />
    </ChecklistAuthGuard>),
});
import { useAuth } from "@/contexts/AuthContext";
function ChecklistAuthGuard({ children }) {
    const { user: authUser, loading: authLoading } = useAuth();
    const navigate = useNavigate();
    const { id: checklistId } = Route.useSearch();
    const [authStatus, setAuthStatus] = useState('session_loading');
    useEffect(() => {
    ...
    if (checklistId) {
        ...
        if (authStatus === 'technical_error') {
          ...
          <div className="flex flex-col items-center gap-4 text-center px-6">
            ...
            <button onClick={() => window.location.reload()} className="mt-4 bg-neutral-900 text-white px-6 py-2 rounded-lg font-medium hover:bg-neutral-800 transition-colors">
              Recarregar
            </button>
          </div>
        </div>);
...
function NovoChecklistPage() {
    ...
    const [publishBlocker, setPublishBlocker] = useState(null);
    ...
    const [isStarted, setIsStarted] = useState(!!checklistId);
    useEffect(() => {
        if (checklistId)
            return;
        ...
        if (draftTitle)
            setTitle(draftTitle);
        if (localStorage.getItem("draft_checklist_started") === "true")
            setIsStarted(true);
    }, [checklistId]);
    ...
    const [btnIcon, setBtnIcon] = useState("arrow-right");
    const [btnIconPosition, setBtnIconPosition] = useState("end");
    ...
    const [customEmailDomainId, setCustomEmailDomainId] = useState(null);
    ...
    const [closedMessageText, setClosedMessageText] = useState("Este formulário está fechado e não está mais aceitando respostas.");
    ...
    const [selectionToolbar, setSelectionToolbar] = useState({ visible: false, top: 0, left: 0, el: null, start: 0, end: 0, isContentEditable: false });
    ...
    const [sendButtonPanelTab, setSendButtonPanelTab] = useState("text");
    const sendButtonPanelRef = useRef(null);
    useEffect(() => {
        if (!sendButtonPanelOpen)
            return;
    ...
    const [settingsActiveTab, setSettingsActiveTab] = useState("geral");
    const sessionChecklistIdRef = useRef(null);
    const [currentChecklistId, setCurrentChecklistId] = useState(null);
    const [profile, setProfile] = useState(null);
    const [shortSlug, setShortSlug] = useState(null);
    ...
    useEffect(() => {
        if (!colorPickerOpen && !highlightPickerOpen && !linkPopoverOpen)
            return;
    ...
    const checkFormatting = (el, start, end) => {
        if (!el)
    ...
    const applyWrap = (before, after, toggle = false) => {
    ...
    useEffect(() => {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
            ...
            if (newUser) {
                ...
                if (isAuthModalOpen && (event === 'SIGNED_IN' || event === 'SIGNED_UP')) {
                ...
                try {
                    ...
                    hasBlocks =
                        Array.isArray(parsed) &&
                            parsed.some((b) => (typeof b?.value === "string" && b.value.trim() !== "") ||
                                (b?.type && b.type !== "text"));
                }
                catch { }
        ...
        const fetchExisting = async () => {
            ...
            try {
                ...
                if (data && !error) {
                    ...
                    setTitle(data.title || "");
                    setBlocks(ensureCameraBlockIds((data.blocks || [{ id: newId(), type: "text", value: "" }])).blocks.map((b) => {
                        if (b.type === 'camera') {
                            // Mantenha apenas os campos legados para remoção se necessário,
                            // mas garanta que vision/criteria não existam no objeto salvo se não forem usados.
                            // Para Camera AI V4/Fase 1, usamos cameraAiPolicy.
                            return b;
                        }
                        return b;
                    }));
                    ...
                    if (data.settings) {
                        ...
                        if (data.custom_email_domain_id)
                            setCustomEmailDomainId(data.custom_email_domain_id);
                    }
                }
            }
            catch (err) {
    ...
    const [slashReplaceForBlockId, setSlashReplaceForBlockId] = useState(null);
    const [pickerOpen, setPickerOpen] = useState(false);
    const [pickerForBlockId, setPickerForBlockId] = useState(null);
    ...
    const [optionsPanelBlockId, setOptionsPanelBlockId] = useState(null);
    const [moveSubmenuOpen, setMoveSubmenuOpen] = useState(false);
    const startBlockDrag = (e, id) => {
        e.preventDefault();
        setDraggingId(id);
        setDragOverId(id);
        setDragOverPos(null);
    ...
    const filteredSections = optionSections
        .map((s) => ({
        ...s,
        options: s.options.filter((o) => o.label.toLowerCase().includes(slashQuery.toLowerCase())),
    }))
        .filter((s) => s.options.length > 0);
    const filteredOptions = filteredSections.flatMap((s) => s.options);
    const createBlockFromLabel = (label) => {
        switch (label) {
    ...
    const insertBlockFromPicker = (label) => {
        ...
        setBlocks((prev) => {
            ...
            if (pickerReplaceMode) {
                next[idx] = { ...block, id: targetBlock.id };
            ...
            if (targetBlock.type === "text" && (!targetBlock.value || targetBlock.value.trim() === "")) {
                ...
                next.splice(idx + 1, 0, { id: newId(), type: "text", value: "" });
                ...
                if (block.type !== "text") {
                    next.splice(idx + 2, 0, { id: newId(), type: "text", value: "" });
                }
                focusBlockId.current = block.id;
            }
            return next;
    ...
    const handleStart = (font) => {
        ...
        setTimeout(() => {
            if (firstId)
                textareaRefs.current[firstId]?.focus();
        }, 0);
    };
    const handleReset = () => {
        setIsStarted(false);
    ...
    const updateSlashFromContent = (blockId, value, caret) => {
    ...
    const insertOption = (label) => {
        ...
        if (slashReplaceForBlockId) {
            ...
            setBlocks((prev) => {
                ...
                const targetBlock = prev[idx];
                setHistory((old) => [JSON.parse(JSON.stringify(prev)), ...old].slice(0, 5));
                const merged = { ...newBlock, id: targetBlock.id };
        ...
        if (!blockId)
            return;
        const ta = textareaRefs.current[blockId];
        if (!ta || slashStartRef.current === null)
            return;
        ...
        setBlocks((prev) => {
            ...
            if (label === "Resposta curta" ||
                ...
                if (label === "Logo" && prev.some(b => b.type === "image" && b.variant === "profile")) {
                    const next = [...prev];
                    next[idx] = { ...block, value: before + after };
                    return next;
                }
                if (label === "Capa" && prev.some(b => b.type === "image" && b.variant === "cover")) {
                    const next = [...prev];
                    next[idx] = { ...block, value: before + after };
                    return next;
                }
                const newBlocks = [...prev];
                const answerBlock = label === "Resposta curta"
    ...
    const saveChecklist = useCallback(async (currentUser, isPublishedOverride, silent = false) => {
        ...
        if (!authUser) {
            if (!silent) {
                ...
                navigate({ to: "/cadastro", search: { redirect: here } });
            }
            return;
        }
        ...
        try {
            ...
            const { blocks: blocksWithIds } = ensureCameraBlockIds(blocks);
            const checklistData = {
                user_id: authUser.id,
                title: (title && title.trim()) ? title.trim() : "Sem título",
                blocks: blocksWithIds.map((b) => {
                    if (b.type === 'camera') {
                        return b;
                    }
                    return b;
            ...
            let serverPublished = data?.is_published === true;
            let serverSlug = data?.custom_slug ?? null;
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            const hasContent = title.trim() !== "" ||
                blocks.length > 1 ||
                (blocks[0]?.type === "text" && blocks[0].value?.trim() !== "");
    ...
    useEffect(() => {
        const handleChecklistAction = (e) => {
            const action = e.detail;
            switch (action) {
                case 'add-logo':
                    if (blocks.some(b => b.type === "image" && b.variant === "profile")) {
                        toast.error("Já existe um bloco de logo no checklist.");
                        return;
                    }
                    setBlocks(prev => {
                        const next = [{ id: newId(), type: "image", src: null, variant: "profile" }, ...prev];
                        return next;
                    });
                    setIsStarted(true);
                    ...
                    break;
                case 'add-cover':
                    if (blocks.some(b => b.type === "image" && b.variant === "cover")) {
                        toast.error("Já existe um bloco de capa no checklist.");
                        return;
                    }
                    setBlocks(prev => {
                        const next = [{ id: newId(), type: "image", src: null, variant: "cover" }, ...prev];
                        return next;
                    });
                    setIsStarted(true);
    ...
    const updateBlock = useCallback((id, patch) => {
        setBlocks((prev) => {
            const newBlocks = prev.map((b) => (b.id === id ? { ...b, ...patch } : b));
    ...
    const removeBlock = useCallback((id) => {
        setBlocks((prev) => {
            ...
            if (prev.length === 1) {
                const block = prev[0];
                if (block.type === "text" && (block.value === "" || block.value === "<br>"))
                    return prev;
                const nid = newId();
                focusBlockId.current = nid;
                setHistory((old) => [JSON.parse(JSON.stringify(prev)), ...old].slice(0, 5));
                return [{ id: nid, type: "text", value: "" }];
            }
            setHistory((old) => [JSON.parse(JSON.stringify(prev)), ...old].slice(0, 5));
    ...
    `}</style>
    <DashboardLayout>
      <div className={`flex-1 flex flex-col transition-colors duration-200 ${theme === "Escuro" ? "bg-[#1a1a1a] text-white" : "bg-white text-neutral-900"}`} style={{
      ...
      {isPreviewMode && (<ChecklistPreview blocks={blocks} title={title} onClose={() => setIsPreviewMode(false)} onPublish={saveChecklist} isPublishing={isPublishing} settings={{
                theme,
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-neutral-100">
        ...
        <div className="flex items-center gap-3 text-sm text-neutral-500">
          {user && (<>
              ...
              <button type="button" onClick={() => setIsHistoryOpen(true)} className="hover:text-neutral-900" aria-label="Histórico">
                <History className="w-4 h-4"/>
              </button>
            </>)}
          <button type="button" onClick={() => setIsSettingsOpen(true)} className="hover:text-neutral-900 flex items-center gap-1.5">
            <Settings className="w-4 h-4"/>
            <span>Configuração</span>
          </button>
          <button type="button" onClick={() => setIsCustomizeOpen((v) => !v)} className={`hover:text-neutral-900 px-2 py-1 rounded-md ${isCustomizeOpen ? "ring-1 ring-blue-400 text-neutral-900" : ""}`}>
            Personalizar
          </button>
          <button type="button" onClick={() => setIsPreviewMode(true)} className="hover:text-neutral-900">
            Pré-visualizar
          </button>
          <button type="button" onClick={() => saveChecklist(undefined, true)} disabled={isPublishing} className="text-xs font-bold bg-[#FF007F] text-white rounded-md px-4 py-1.5 hover:opacity-90 transition-opacity disabled:opacity-50 shadow-sm">
            {isPublishing ? "Publicando..." : "Publicar"}
          </button>
        </div>
      </header>
      ...
      {/* Editor */}
      <main className="flex-1 flex flex-col items-center pt-24 pb-20 overflow-x-hidden" style={{
            backgroundColor: theme === "Escuro" ? "#1a1a1a" : bgColor
        }}>
        <div className="w-full">
            ...
            const profileBlock = blocks.find((b) => b.type === "image" && b.variant === "profile");
            const coverBlock = blocks.find((b) => b.type === "image" && b.variant === "cover");
            if (!profileBlock && !coverBlock)
                return null;
            return (<div className="w-full mb-10">
                  <div className={`relative ${coverBlock && profileBlock ? "mb-16" : ""}`}>
                    {coverBlock && (<div className="relative w-full -mt-24 h-80">
                        <ImageBlock key={coverBlock.id} src={coverBlock.src} variant="cover" position={coverBlock.position} onPositionChange={(pos) => updateBlock(coverBlock.id, { position: pos })} onChange={(src) => updateBlock(coverBlock.id, { src })} onRemove={() => removeBlock(coverBlock.id)}/>
                      </div>)}
                    {profileBlock && (<div className={`${coverBlock ? "absolute left-1/2 bottom-0 -translate-x-1/2 translate-y-1/2" : "flex justify-center mb-2"} z-10`}>
                        <ImageBlock key={profileBlock.id} src={profileBlock.src} variant="profile" logoSettings={{
                        width: logoWidth,
                        height: logoHeight,
                        radius: logoRadius
                    }} position={profileBlock.position} onPositionChange={(pos) => updateBlock(profileBlock.id, { position: pos })} onChange={(src) => updateBlock(profileBlock.id, { src })} onRemove={() => removeBlock(profileBlock.id)}/>
                      </div>)}
                  </div>
                </div>);
        })()}

          <div className="w-full mx-auto px-6" style={{
            maxWidth: pageWidth,
            color: textColor
        }}>
            <div onMouseEnter={() => setIsTitleAreaHovered(true)} onMouseLeave={() => setIsTitleAreaHovered(false)} className="group relative">
              {isStarted && (<div className={`flex items-center gap-5 mb-2 text-sm text-neutral-500 transition-all duration-300 ${isTitleAreaHovered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1 pointer-events-none"}`}>
                  <div className="flex items-center gap-2">
                    <Switch checked={blocks.some((b) => b.type === "image" && b.variant === "profile")} onCheckedChange={(checked) => {
                setBlocks((prev) => {
                    setHistory((old) => [JSON.parse(JSON.stringify(prev)), ...old].slice(0, 5));
                    if (!checked) {
                        return prev.filter((b) => !(b.type === "image" && b.variant === "profile"));
                    }
                    else {
                        if (prev.some(b => b.type === "image" && b.variant === "profile"))
                            return prev;
                        return [
                            { id: newId(), type: "image", src: null, variant: "profile" },
                            ...prev,
                  ...
                  <div className="flex items-center gap-2">
                    <Switch checked={blocks.some((b) => b.type === "image" && b.variant === "cover")} onCheckedChange={(checked) => {
                setBlocks((prev) => {
                    setHistory((old) => [JSON.parse(JSON.stringify(prev)), ...old].slice(0, 5));
                    if (!checked) {
                        return prev.filter((b) => !(b.type === "image" && b.variant === "cover"));
                    }
                    else {
                        if (prev.some(b => b.type === "image" && b.variant === "cover"))
                            return prev;
                        const profileIdx = prev.findIndex((b) => b.type === "image" && b.variant === "profile");
                        const newBlock = { id: newId(), type: "image", src: null, variant: "cover" };
                        const next = [...prev];
                  ...
                  <button type="button" onClick={() => setIsCustomizeOpen((v) => !v)} className="flex items-center gap-1.5 hover:text-neutral-900 transition-colors">
                    <Palette className="w-4 h-4" strokeWidth={1.75}/>
                    <span>Personalizar</span>
                  </button>
                </div>)}

              <input ref={titleInputRef} type="text" value={title} onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter" && !isStarted && !title.trim()) {
                e.preventDefault();
                handleStart();
            }
        }} placeholder="Título do checklist" className="w-full text-4xl font-bold placeholder:text-neutral-300 border-none outline-none bg-transparent" style={{ color: textColor }} autoFocus/>
            </div>

          {!isStarted ? (<>
              {!isTemplatesOpen ? (<div className="mt-8 space-y-2 text-sm text-neutral-500">
                  <div className="flex items-center gap-2 px-2 py-1 rounded hover:bg-neutral-50 cursor-text transition-colors" onClick={() => handleStart()} onKeyDown={handleKeyDown} tabIndex={0}>
                    <FileText className="w-4 h-4"/>
                    <span>Pressione Enter para começar do zero</span>
                  </div>
                  <div className="flex items-center gap-2 px-2 py-1 rounded hover:bg-neutral-50 cursor-pointer transition-colors" onClick={() => setIsTemplatesOpen(true)}>
                    <LayoutTemplate className="w-4 h-4"/>
                    <span>Usar um modelo</span>
                  </div>
                </div>) : (<div className="mt-8 p-6 bg-neutral-50 rounded-xl border border-neutral-100 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-semibold text-neutral-900">Modelos Disponíveis</h3>
                    <button onClick={() => setIsTemplatesOpen(false)} className="text-xs text-neutral-500 hover:text-neutral-900">
                      Voltar
                    </button>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    {["Poppins", "Montserrat", "Roboto"].map((font) => (<button key={font} onClick={() => handleStart(font)} className="flex flex-col items-center gap-3 p-4 bg-white rounded-lg border border-neutral-200 hover:border-pink-300 hover:shadow-md transition-all group">
              ...
              <div className="mt-10 grid grid-cols-2 gap-x-12 gap-y-3 text-sm">
            ...
            </>) : (<div className="mt-8 space-y-3 pb-32" data-workspace style={{ fontFamily: selectedFont ? `'${selectedFont}', sans-serif` : undefined }} onClick={(e) => {
               ...
               {blocks.map((block, i) => {
                ...
                const inner = (() => {
                    if (block.type === "text") {
                        ...
                        <EditableBlock key={block.id} onBlur={() => {
                                setTimeout(() => {
                                    setSlashMenuOpen(false);
                                }, 150);
                            }} onChange={(html) => {
                                updateBlock(block.id, { value: html });
                            ...
                            }} onKeyDown={(e) => handleContentKeyDown(e)} onKeyDownCapture={(e) => {
                                if (e.key === "Backspace") {
                                    const selection = window.getSelection();
                                    if (!selection)
                                        return;
                                    ...
                                    const innerText = textareaRefs.current[block.id]?.innerText || "";
                                    ...
                                    setBlocks(prev => {
                                        ...
                                        next.splice(idx + 1, 0, { id: nextId, type: "text", value: after });
                                        return next;
                                    });
                                ...
                                return block.value;
                            })()} className="w-full text-lg border-none outline-none bg-transparent leading-relaxed min-h-[1.6em] whitespace-pre-wrap break-words [overflow-wrap:anywhere]" style={{ color: "inherit" }}/>
                        {(!block.value || block.value === "<br>") && (<div className="absolute inset-0 pointer-events-none text-neutral-400 text-lg opacity-50">
                    ...
                    if (block.type === "short-answer" ||
                        ...
                        block.type === "date" ||
                        block.type === "time") {
                        return (<div key={block.id} onFocus={() => setActiveBlockId(block.id)} className="group flex flex-col gap-2 border border-transparent transition-colors">
                     <input type="text" ref={(el) => {
                                if (el)
                                    textareaRefs.current[block.id] = el;
                            }} value={block.placeholder} onChange={(e) => updateBlock(block.id, { placeholder: e.target.value })} onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                    e.preventDefault();
                                    const idx = blocks.findIndex(b => b.id === block.id);
                                    const nextId = newId();
                                    setBlocks(prev => {
                                        setHistory((old) => [JSON.parse(JSON.stringify(prev)), ...old].slice(0, 5));
                                        const next = [...prev];
                                        next.splice(idx + 1, 0, { id: nextId, type: "text", value: "" });
                                        return next;
                                    });
                                    focusBlockId.current = nextId;
                                }
                            }} placeholder="Texto de exemplo (placeholder)" className="w-full bg-white border border-neutral-200 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-pink-500/20 transition-all text-sm placeholder:text-neutral-400 shadow-sm" style={{ color: textColor }}/>
                    </div>);
                    }
                    ...
                    <textarea ref={(el) => {
                            ...
                            }} placeholder="Texto de exemplo (placeholder)" rows={4} className="w-full bg-white border border-neutral-200 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-pink-500/20 transition-all text-sm placeholder:text-neutral-400 resize-none min-h-24 shadow-sm" style={{ color: textColor }}/>
                  </div>);
                    }
                    if (block.type === "checkboxes") {
                        return (<div key={block.id} onFocus={() => setActiveBlockId(block.id)} className="group w-full space-y-2">
                      {block.options.map((opt, oi) => (<div key={opt.id} className="flex items-center gap-3 p-3 rounded-lg bg-white border border-neutral-100 shadow-sm">
                          <button type="button" onClick={() => {
                                    const newOpts = block.options.map((o) => o.id === opt.id ? { ...o, checked: !o.checked } : o);
                                    updateBlock(block.id, { options: newOpts });
                                }} aria-label={opt.checked ? "Desmarcar" : "Marcar"} className={`flex items-center justify-center w-4 h-4 rounded-sm border transition-colors workspace-accent-border ${opt.checked
                                    ? "workspace-accent-bg text-white"
                                    : "bg-transparent border-neutral-300 hover:border-neutral-400"}`} style={{ backgroundColor: opt.checked ? "var(--accent-color)" : undefined, borderColor: opt.checked ? "var(--accent-color)" : undefined }}>
                            {opt.checked && (<svg viewBox="0 0 16 16" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                          ...
                          <input type="text" ref={(el) => {
                                    ...
                                    if (e.key === "Enter") {
                                        ...
                                        setTimeout(() => {
                                            ...
                                            if (inputs && inputs[currentIdx + 1]) {
                                                ...
                                                inputs[currentIdx + 1].scrollIntoView({ behavior: 'smooth', block: 'center' });
                                            }
                                        }, 0);
                                    }
                                ...
                                }} placeholder={`Opção ${oi + 1}`} className="flex-1 bg-transparent border-none outline-none text-sm placeholder:text-neutral-400 font-medium" style={{ color: textColor }}/>
                        </div>))}
                      <button type="button" onClick={() => {
                    ...
                    if (block.type === "dropdown") {
                        ...
                        return (<div key={block.id} onFocus={() => setActiveBlockId(block.id)} className="group w-full space-y-2 relative">
                       <button type="button" ref={(el) => {
                                if (el)
                                    textareaRefs.current[block.id] = el;
                            }} onClick={() => setOpenDropdownId(isOpen ? null : block.id)} onKeyDown={(e) => {
                                if (e.key === "Enter") {
                          ...
                          <span className="text-sm" style={{ color: selected && selected.value ? textColor : "#A3A3A3" }}>
                            {selected && selected.value ? selected.value : "Selecione uma opção"}
                          </span>
                          <ChevronDown className={`w-4 h-4 text-neutral-400 transition-transform ${isOpen ? "rotate-180" : ""}`}/>
                        </button>
                      {isOpen && (<div className="w-full border border-neutral-100 rounded-md bg-white shadow-sm p-2 space-y-1.5">
                          {block.options.map((opt, oi) => (<div key={opt.id} className={`flex items-center gap-2 w-full border rounded-lg px-3 py-2 transition-colors cursor-pointer ${block.selectedId === opt.id
                                        ? "border-blue-400 bg-blue-50/50"
                                        : "border-neutral-200 bg-neutral-50/50 hover:border-neutral-300"}`} onClick={() => updateBlock(block.id, { selectedId: opt.id })}>
                              <input type="text" value={opt.value} onClick={(e) => e.stopPropagation()} onChange={(e) => {
                                        const newOpts = block.options.map((o) => o.id === opt.id ? { ...o, value: e.target.value } : o);
                                        updateBlock(block.id, { options: newOpts });
                                    }} onKeyDown={(e) => {
                                        if (e.key === "Backspace") {
                                            ...
                                            if (target.value === "") {
                                                ...
                                                if (block.options.length > 1) {
                                                    const currentIdx = block.options.findIndex(o => o.id === opt.id);
                                                    const newOpts = block.options.filter(o => o.id !== opt.id);
                                                    const updateData = { options: newOpts };
                                                    if (block.selectedId === opt.id) {
                                                        updateData.selectedId = null;
                                                    }
                                    ...
                                    }} placeholder={`Opção ${oi + 1}`} className="flex-1 bg-transparent border-none outline-none text-sm placeholder:text-neutral-400" style={{ color: textColor }}/>
                            </div>))}
                    ...
                    if (block.type === "multi-select") {
                      ...
                      <button type="button" ref={(el) => {
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 flex-1">
                          {selectedOpts.length === 0 ? (<span className="text-sm text-neutral-400">Selecione uma ou mais opções</span>) : (selectedOpts.map((opt) => {
                                const c = colorFor(opt.id);
                                return (<span key={opt.id} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium border ${c.bg} ${c.text} ${c.border}`}>
                                  {opt.value}
                                  <span role="button" tabIndex={0} onClick={(e) => {
                                        e.stopPropagation();
                                        toggleSelected(opt.id);
                                    }} className="cursor-pointer hover:opacity-70" aria-label="Remover">
                                    ×
                                  </span>
                                </span>);
                            }))}
                      ...
                      {isOpen && (<div className="w-full border border-neutral-200 rounded-md bg-white shadow-sm p-2 space-y-2">
                          {block.options.map((opt, oi) => {
                                    const c = colorFor(opt.id);
                                    const isSelected = block.selectedIds.includes(opt.id);
                                    return (<div key={opt.id} onClick={() => toggleSelected(opt.id)} className={`flex items-center gap-2 w-full border rounded-md px-3 py-2 cursor-pointer transition-colors ${isSelected ? `${c.bg} ${c.border}` : `${c.bg} ${c.border} opacity-50 grayscale-[0.5] hover:opacity-100 hover:grayscale-0`}`}>
                                <input type="text" value={opt.value} onClick={(e) => e.stopPropagation()} onChange={(e) => {
                                            const newOpts = block.options.map((o) => o.id === opt.id ? { ...o, value: e.target.value } : o);
                                            updateBlock(block.id, { options: newOpts });
                                        }} onKeyDown={(e) => {
                                            if (e.key === "Backspace") {
                                                ...
                                                if (target.value === "") {
                                                    ...
                                                    if (block.options.length > 1) {
                                                        ...
                                                        updateBlock(block.id, { options: newOpts, selectedIds: newSelected });
                                                        setTimeout(() => {
                                                            const parent = blockRefs.current[block.id];
                                                            const inputs = parent?.querySelectorAll('input[type="text"]');
                                                            const focusIdx = Math.max(0, currentIdx - 1);
                                        ...
                                        }} placeholder={`Opção ${oi + 1}`} className={`flex-1 bg-transparent border-none outline-none text-sm placeholder:text-neutral-400`} style={{ color: isSelected ? undefined : textColor }}/>
                              </div>);
                                })}
                          <button type="button" onClick={(e) => {
                                    e.stopPropagation();
                    ...
                    if (block.type === "ranking") {
                        ...
                        return (<div key={block.id} ref={(el) => {
                                if (el)
                                    textareaRefs.current[block.id] = el;
                            }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={(e) => {
                                if (e.key === "Backspace") {
                                    e.preventDefault();
                                    removeBlock(block.id);
                                }
                       ...
                       {[...selectedOpts, ...block.options.filter((o) => !block.selectedIds.includes(o.id))].map((opt, oi) => {
                                ...
                                return (<div key={opt.id} onClick={() => toggleRanking(opt.id)} className={`flex items-center gap-2 w-full border rounded-md px-3 py-2 cursor-pointer transition-all duration-300 workspace-accent-border ${isSelected
                                        ? "bg-opacity-10 shadow-sm"
                                        : "border-neutral-200 bg-white hover:border-neutral-300"}`} style={{ backgroundColor: isSelected ? "var(--accent-color)" : undefined, borderColor: isSelected ? "var(--accent-color)" : undefined }}>
                            <div className={`flex items-center justify-center w-5 h-5 rounded border text-[10px] font-bold transition-colors workspace-accent-border ${isSelected
                                        ? "workspace-accent-bg text-white"
                            ...
                            <input type="text" ref={(el) => {
                                    ...
                                    }} value={opt.value} onClick={(e) => e.stopPropagation()} onChange={(e) => {
                                        const newOpts = block.options.map((o) => o.id === opt.id ? { ...o, value: e.target.value } : o);
                                        updateBlock(block.id, { options: newOpts });
                                    }} onKeyDown={(e) => {
                                        if (e.key === "Backspace") {
                                    ...
                                    }} placeholder={`Opção ${oi + 1}`} className="flex-1 bg-transparent border-none outline-none text-sm text-neutral-700 placeholder:text-neutral-400"/>
                          </div>);
                            })}
                      <button type="button" onClick={() => {
                                const newOpts = [...block.options, { id: newId(), value: "" }];
                    ...
                    if (block.type === "file-upload") {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={(e) => {
                                if (e.key === "Backspace") {
                                    e.preventDefault();
                                    removeBlock(block.id);
                                }
                            }} className="group w-full outline-none">
                      <div aria-disabled className="relative w-full h-16 border border-neutral-200 dark:border-neutral-700 rounded-lg bg-neutral-50/50 dark:bg-neutral-800/50 flex items-center px-4 overflow-hidden select-none">
                    ...
                    if (block.type === "linear-scale") {
                        return (<div key={block.id} ref={(el) => {
                                if (el)
                                    textareaRefs.current[block.id] = el;
                            }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={() => { }} className="group w-full outline-none">
                      <div className="flex flex-wrap items-center gap-2">
                        {Array.from({ length: 11 }, (_, n) => {
                                const isSelected = block.selected === n;
                                return (<button key={n} type="button" onClick={() => updateBlock(block.id, { selected: isSelected ? null : n })} className={`flex items-center justify-center w-10 h-10 rounded-md border text-sm font-medium transition-colors workspace-accent-border ${isSelected
                                        ? "bg-white text-neutral-700 ring-2"
                                        : "bg-white border-neutral-200 text-neutral-700 hover:border-neutral-300"}`} style={{ borderColor: isSelected ? "var(--accent-color)" : undefined, boxShadow: isSelected ? "0 0 0 2px var(--accent-color)" : undefined }}>
                              {n}
                            </button>);
                            })}
                      </div>
                    ...
                    if (block.type === "matrix") {
                        return (<div key={block.id} className="group w-full space-y-2">
                        ...
                        <table className="w-full border-collapse">
                          <thead>
                            <tr>
                              ...
                              {block.columns.map((col, idx) => (<th key={col.id} className="border-b border-r border-neutral-200 last:border-r-0 p-2">
                                  <input type="text" ref={(el) => {
                                    if (el && idx === 0) {
                                        textareaRefs.current[block.id] = el;
                                    }
                                }} value={col.label} onChange={(e) => {
                                    const newCols = block.columns.map((c) => c.id === col.id ? { ...c, label: e.target.value } : c);
                                    updateBlock(block.id, { columns: newCols });
                                }} onKeyDown={() => { }} className="w-full bg-transparent border-none outline-none text-center text-sm placeholder:text-neutral-400" style={{ color: textColor }}/>
                                </th>))}
                            </tr>
                          </thead>
                          <tbody>
                            {block.rows.map((row) => (<tr key={row.id}>
                                <td className="border-t border-r border-neutral-200 p-2 w-32">
                                  <input type="text" value={row.label} onChange={(e) => {
                                    const newRows = block.rows.map((r) => r.id === row.id ? { ...r, label: e.target.value } : r);
                                    updateBlock(block.id, { rows: newRows });
                                }} className="w-full bg-transparent border-none outline-none text-sm placeholder:text-neutral-400" style={{ color: textColor }}/>
                                </td>
                                {block.columns.map((col) => {
                                    const selected = block.selections[row.id] === col.id;
                                    return (<td key={col.id} className="border-t border-r border-neutral-200 last:border-r-0 p-2 text-center">
                                      <button type="button" onClick={() => {
                                            ...
                                            updateBlock(block.id, { selections: newSel });
                                        }} className={`inline-flex items-center justify-center w-5 h-5 rounded-full border-2 transition-colors ${selected
                                            ? "border-neutral-700"
                                            : "border-neutral-300 hover:border-neutral-400"}`} aria-label="Selecionar">
                    ...
                    if (block.type === "rating") {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={() => { }} className="outline-none">
                      <RatingStars value={block.value} onChange={(v) => updateBlock(block.id, { value: v })}/>
                    </div>);
                    }
                    if (block.type === "signature") {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={() => { }} className="outline-none">
                      <SignaturePad dataUrl={block.dataUrl} onChange={(v) => updateBlock(block.id, { dataUrl: v })}/>
                    </div>);
                    }
                    if (block.type === "page-break") {
                        const pageNum = blocks.slice(0, blocks.findIndex((b) => b.id === block.id) + 1)
                            .filter((b) => b.type === "page-break").length + 1;
                        return (<div key={block.id} className="w-full flex items-center gap-3 py-2 text-neutral-400 text-sm">
                    ...
                    if (block.type === "heading-1" ||
                        ...
                        block.type === "label") {
                        const sizeClass = block.type === "heading-1"
                            ? "text-5xl font-bold"
                            : block.type === "heading-2"
                                ? "text-4xl font-bold"
                                : block.type === "heading-3"
                                    ...
                                    : block.type === "observation"
                                        ? "text-2xl font-bold"
                                        : "text-xl font-medium";
                        const placeholderText = block.type === "heading-1"
                            ? "Título 1"
                            : block.type === "heading-2"
                                ? "Título 2"
                                : block.type === "heading-3"
                                    ...
                                    : block.type === "observation"
                                        ? "Observação"
                                        : "Rótulo";
                        return (<input key={block.id} ref={(el) => {
                                if (el)
                                    textareaRefs.current[block.id] = el;
                            }} type="text" onFocus={() => setActiveBlockId(block.id)} value={block.value} onChange={(e) => updateBlock(block.id, { value: e.target.value })} onKeyDown={(e) => {
                                if (e.key === "Backspace") {
                                    const target = e.target;
                            ...
                            }} placeholder={placeholderText} className={`w-full bg-transparent border-none outline-none placeholder:text-neutral-300 ${sizeClass}`} style={{ color: textColor }}/>);
                    }
                    ...
                    if (block.type === "image" && !block.variant) {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={() => { }} className="outline-none">
                      <div aria-disabled className="flex flex-col items-center justify-center gap-2 w-full border-2 border-dashed border-neutral-200 rounded-md px-4 py-8 bg-neutral-50/50 select-none">
                    ...
                    if (block.type === "video") {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={() => { }} className="outline-none">
                      <VideoBlock src={block.src} onChange={(src) => updateBlock(block.id, { src })} onRemove={() => removeBlock(block.id)}/>
                    </div>);
                    }
                    if (block.type === "embed") {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} onKeyDown={() => { }} className="outline-none">
                      <EmbedBlock src={block.src} onChange={(src) => updateBlock(block.id, { src })} onRemove={() => removeBlock(block.id)}/>
                    </div>);
                    }
                    if (block.type === "task-list") {
                       ...
                       {block.options.map((opt, oi) => (<div key={opt.id} className="flex items-center gap-3 p-3 rounded-lg bg-white border border-neutral-100 shadow-sm">
                           <button type="button" onClick={() => {
                                    const newOpts = block.options.map((o) => o.id === opt.id ? { ...o, checked: !o.checked } : o);
                                    updateBlock(block.id, { options: newOpts });
                                }} className="flex items-center justify-center w-4 h-4 rounded-sm border transition-colors" style={{ backgroundColor: opt.checked ? "var(--accent-color)" : undefined, borderColor: opt.checked ? "var(--accent-color)" : "#d4d4d4" }}>
                             {opt.checked && (<svg viewBox="0 0 16 16" className="w-3 h-3 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                 <polyline points="3 8 7 12 13 4"/>
                               </svg>)}
                           </button>
                           <input type="text" ref={(el) => { if (el && oi === block.options.length - 1)
                                textareaRefs.current[block.id] = el; }} value={opt.value} onChange={(e) => {
                                    const newOpts = block.options.map((o) => o.id === opt.id ? { ...o, value: e.target.value } : o);
                                    updateBlock(block.id, { options: newOpts });
                                }} onKeyDown={(e) => {
                                    if (e.key === "Enter") {
                                ...
                                }} placeholder={`Tarefa ${oi + 1}`} className={`flex-1 bg-transparent border-none outline-none text-sm placeholder:text-neutral-400 ${opt.checked ? "line-through text-neutral-400" : ""}`} style={{ color: opt.checked ? undefined : textColor }}/>
                         </div>))}
                       <button type="button" onClick={() => {
                                const newOpts = [...block.options, { id: newId(), value: "", checked: false }];
                                updateBlock(block.id, { options: newOpts });
                    ...
                    if (block.type === "counter") {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} className="flex items-center gap-3 p-3 rounded-lg bg-white border border-neutral-200 shadow-sm w-fit outline-none">
                       <button type="button" onClick={() => updateBlock(block.id, { value: Math.max(block.min, block.value - block.step) })} className="w-9 h-9 rounded-md border border-neutral-200 bg-white hover:bg-neutral-50 flex items-center justify-center text-lg font-semibold text-neutral-700">
                         −
                       </button>
                       <input type="number" value={block.value} onChange={(e) => updateBlock(block.id, { value: Number(e.target.value) || 0 })} className="w-20 text-center text-lg font-semibold bg-transparent border-none outline-none" style={{ color: textColor }}/>
                       <button type="button" onClick={() => updateBlock(block.id, { value: Math.min(block.max, block.value + block.step) })} className="w-9 h-9 rounded-md border border-neutral-200 bg-white hover:bg-neutral-50 flex items-center justify-center text-lg font-semibold text-neutral-700">
                         +
                       </button>
                     </div>);
                    }
                    if (block.type === "currency") {
                        return (<div key={block.id} onFocus={() => setActiveBlockId(block.id)} className="group flex flex-col gap-2">
                       <div className="flex items-center w-full bg-white border border-neutral-200 rounded-lg shadow-sm focus-within:ring-2 focus-within:ring-pink-500/20">
                         ...
                         <input type="text" ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} value={block.placeholder} onChange={(e) => updateBlock(block.id, { placeholder: e.target.value })} placeholder="0,00" className="flex-1 bg-transparent border-none outline-none px-2 py-3 text-sm placeholder:text-neutral-400" style={{ color: textColor }}/>
                       </div>
                     </div>);
                    }
                    if (block.type === "audio") {
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} className="outline-none p-4 rounded-lg bg-white border border-neutral-200 shadow-sm">
                       {block.src ? (<div className="flex items-center gap-3">
                           <audio src={block.src} controls className="flex-1"/>
                           <button type="button" onClick={() => updateBlock(block.id, { src: null })} className="text-xs text-neutral-500 hover:text-neutral-700">
                             Remover
                           </button>
                         </div>) : (<label className="flex items-center gap-3 cursor-pointer text-neutral-500 hover:text-neutral-700">
                    ...
                    if (block.type === "qr-code") {
                        const qrSrc = block.value
                            ...
                            : null;
                        return (<div key={block.id} ref={(el) => { if (el)
                            textareaRefs.current[block.id] = el; }} tabIndex={0} onFocus={() => setActiveBlockId(block.id)} className="outline-none p-4 rounded-lg bg-white border border-neutral-200 shadow-sm flex flex-col items-center gap-3">
                       <input type="text" value={block.value} onChange={(e) => updateBlock(block.id, { value: e.target.value })} placeholder="Texto ou URL para gerar QR Code" className="w-full bg-white border border-neutral-200 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-pink-500/20 text-sm placeholder:text-neutral-400" style={{ color: textColor }}/>
                       {qrSrc ? (<img src={qrSrc} alt="QR Code" className="w-40 h-40"/>) : (<div className="w-40 h-40 flex items-center justify-center text-neutral-300">
                    ...
                    if (block.type === "multiple-choice") {
                        return (<div key={block.id} onFocus={() => setActiveBlockId(block.id)} className="group w-full space-y-2">
                       {block.options.map((opt, oi) => (<div key={opt.id} className="flex items-center gap-2 w-full border border-neutral-200 rounded-md px-3 py-2 bg-white hover:border-neutral-300 transition-colors">
                        <span className="flex items-center justify-center w-6 h-6 rounded workspace-accent-bg text-white text-xs font-semibold" style={{ backgroundColor: "var(--accent-color)" }}>
                         ...
                         <input type="text" ref={(el) => {
                                ...
                                }} placeholder={`Opção ${oi + 1}`} className="flex-1 bg-neutral-50/50 border border-neutral-200 rounded-lg px-4 py-2 outline-none focus:border-neutral-300 transition-all text-sm placeholder:text-neutral-400" style={{ color: textColor }}/>
                      </div>))}
                    <button type="button" onClick={() => {
                                const newOpts = [...block.options, { id: newId(), value: "" }];
                                updateBlock(block.id, { options: newOpts });
                    ...
                    if (block.type === "camera") {
                        const isActive = activeBlockId === block.id;
                        return (<CameraBlockEditor block={block} isActive={isActive} currentChecklistId={currentChecklistId} updateBlock={updateBlock} removeBlock={removeBlock} setActiveBlockId={setActiveBlockId} textColor={textColor} textareaRefs={textareaRefs}/>);
                    }
                    return null;
                })();
                if (inner === null || inner === undefined)
                    return null;
                return (<div key={block.id} ref={(el) => { blockRefs.current[block.id] = el; }} onFocus={() => setActiveBlockId(block.id)} onClick={() => setActiveBlockId(block.id)} onBlur={(e) => {
                    ...
                    }} className={`group relative flex items-start gap-1 transition-colors ${draggingId === block.id ? "bg-pink-500/30 rounded-md" : ""} ${draggingId && dragOverId === block.id && dragOverPos === "before" && draggingId !== block.id
                    ...
                    <div className={`absolute -left-20 top-1.5 flex items-center gap-1 transition-all duration-300 text-neutral-600 ${activeBlockId === block.id ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
                      <button type="button" onClick={() => removeBlock(block.id)} aria-label="Apagar" className="hover:text-neutral-900">
                      ...
                      <button type="button" onClick={() => {
                        ...
                        setPickerQuery("");
                        setPickerOpen(true);
                    }} aria-label="Adicionar" className="hover:text-neutral-900">
                        <Plus className="w-4 h-4" strokeWidth={2.5}/>
                      </button>
                      <div className="relative group/options">
                        <button type="button" onPointerDown={(e) => startBlockDrag(e, block.id)} onClick={() => setOptionsPanelBlockId(optionsPanelBlockId === block.id ? null : block.id)} className="cursor-grab active:cursor-grabbing hover:text-neutral-900 touch-none flex items-center" aria-label="Mover e Opções" data-grip-for={block.id}>
                          <GripVertical className="w-4 h-4" strokeWidth={2.5}/>
                        </button>
                        
                        {optionsPanelBlockId === block.id && (<>
                            <div className="fixed inset-0 z-[190]" onClick={() => setOptionsPanelBlockId(null)}/>
                            <div className="absolute left-0 top-full mt-1 z-[200] w-64 bg-white dark:bg-neutral-800 rounded-lg shadow-xl border border-neutral-200 dark:border-neutral-700 py-2 animate-in fade-in zoom-in-95 duration-200">
                              <div className="px-3 py-1.5 space-y-3">
                                <div className="flex items-center justify-between">
                                  ...
                                  <Switch checked={block.required !== false} onCheckedChange={(val) => updateBlock(block.id, { required: val })} className="scale-75"/>
                                </div>
                                <button type="button" onClick={() => {
                            setSlashQuery("");
                            setSlashIndex(0);
                        ...
                        }} aria-label="Mover para cima" className="p-1 rounded hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300">
                            ...
                            setBlocks((prev) => {
                        ...
                        }} aria-label="Mover para baixo" className="p-1 rounded hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300">
                                ...
                                <div className="space-y-2">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <Switch checked={!!block.minChars} onCheckedChange={(val) => updateBlock(block.id, { minChars: val ? 1 : undefined })} className="scale-75"/>
                                  </div>
                                  {!!block.minChars && (<div className="flex items-center gap-1.5 pl-1">
                                      <button type="button" onClick={() => {
                                const cur = Number(block.minChars) || 1;
                                updateBlock(block.id, { minChars: Math.max(1, cur - 1) });
                            }} className="w-6 h-6 flex items-center justify-center rounded border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 text-sm" aria-label="Diminuir mínimo">
                                        −
                                      </button>
                                      <input type="number" min={1} value={block.minChars ?? 1} onChange={(e) => {
                                const n = parseInt(e.target.value, 10);
                                updateBlock(block.id, { minChars: Number.isFinite(n) && n >= 1 ? n : 1 });
                                      ...
                                      <button type="button" onClick={() => {
                                const cur = Number(block.minChars) || 1;
                                updateBlock(block.id, { minChars: cur + 1 });
                            }} className="w-6 h-6 flex items-center justify-center rounded border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 text-sm" aria-label="Aumentar mínimo">
                                        +
                                      </button>
                                    </div>)}
                                </div>
                                <div className="space-y-2">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <Switch checked={!!block.maxChars} onCheckedChange={(val) => updateBlock(block.id, { maxChars: val ? 100 : undefined })} className="scale-75"/>
                                  </div>
                                  {!!block.maxChars && (<div className="flex items-center gap-1.5 pl-1">
                                      <button type="button" onClick={() => {
                                const cur = Number(block.maxChars) || 1;
                                updateBlock(block.id, { maxChars: Math.max(1, cur - 1) });
                            }} className="w-6 h-6 flex items-center justify-center rounded border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 text-sm" aria-label="Diminuir máximo">
                                        −
                                      </button>
                                      <input type="number" min={1} value={block.maxChars ?? 100} onChange={(e) => {
                                const n = parseInt(e.target.value, 10);
                                updateBlock(block.id, { maxChars: Number.isFinite(n) && n >= 1 ? n : 1 });
                            }} className="flex-1 h-6 text-xs text-center rounded border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-700 dark:text-neutral-200 focus:outline-none focus:ring-1 focus:ring-blue-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
                                      <button type="button" onClick={() => {
                                const cur = Number(block.maxChars) || 1;
                                updateBlock(block.id, { maxChars: cur + 1 });
                            }} className="w-6 h-6 flex items-center justify-center rounded border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 text-sm" aria-label="Aumentar máximo">
                                        +
                                      </button>
                                    </div>)}
                                </div>
                            ...
                            setOptionsPanelBlockId(null);
                              ...
                              <button onClick={() => setOptionsPanelBlockId(null)} className="w-full px-3 py-2 text-left text-sm text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-700 flex items-center justify-between transition-colors">
                                <div className="flex items-center gap-2.5">
                        ...
                        {slashReplaceForBlockId === block.id && slashMenuOpen && filteredOptions.length > 0 && (<>
                            <div className="fixed inset-0 z-[190]" onClick={() => { setSlashMenuOpen(false); setSlashReplaceForBlockId(null); setSlashQuery(""); }}/>
                            <div className="absolute left-0 top-full mt-1 z-[200] w-64 max-h-80 overflow-y-auto bg-white rounded-lg border border-neutral-200 shadow-lg py-1 animate-in fade-in zoom-in-95 duration-200">
                              {(() => {
                            let runningIdx = 0;
                            return filteredSections.map((section) => (<div key={section.title}>
                                    <p className="px-3 py-1.5 text-xs font-medium text-neutral-500">{section.title}</p>
                                    {section.options.map((opt) => {
                                    const Icon = opt.icon;
                                    const idx = runningIdx++;
                                    return (<button key={opt.label} type="button" onMouseDown={(e) => {
                                            e.preventDefault();
                                            insertOption(opt.label);
                                        }} onMouseEnter={() => setSlashIndex(idx)} className={`w-full flex items-center gap-2.5 px-3 py-1.5 text-sm text-left transition-all ${idx === slashIndex ? "bg-neutral-100 text-neutral-900" : "text-neutral-700 hover:bg-neutral-50"}`}>
                    ...
                    <div className={`flex-1 min-w-0 ${draggingId === block.id ? "opacity-0 pointer-events-none" : ""}`}>
                      {(() => {
                        ...
                        return (<div className="space-y-1.5">
                            {showSubtitle && (<div className="flex items-start gap-2 group/subtitle">
                                <textarea value={subtitle} onChange={(e) => {
                                    const currentTitle = String(block.title ?? "");
                                ...
                                }} onFocus={() => setActiveBlockId(block.id)} onKeyDown={(e) => {
                                    if (e.key === "Backspace") {
                                        const target = e.target;
                                ...
                                }} placeholder="Digite uma pergunta" rows={1} className="w-full bg-transparent border-none outline-none text-lg placeholder:text-neutral-400 font-bold resize-none px-0 py-0" style={{ minHeight: "1.5em", color: textColor }} onInput={(e) => {
                                ...
                                }} className={`text-lg font-bold select-none transition-all ml-1 ${block.required !== false
                                    ? "text-[#FF007F] opacity-100"
                                    : "text-neutral-300 opacity-0 group-hover/subtitle:opacity-100 hover:opacity-100"}`} style={{ marginTop: "2px" }} title={block.required !== false ? "Remover obrigatoriedade" : "Tornar obrigatória"}>
                                  *
                                </button>
                              </div>)}
                            {inner}
                 ...
                 {isStarted && (<div ref={sendButtonPanelRef} className="mt-6 mb-4 flex flex-col items-start gap-3 relative">
                    <div className="flex items-center gap-2 group">
                      <div className="relative" onMouseEnter={() => setIsBtnHovered(true)} onMouseLeave={() => setIsBtnHovered(false)}>
                        <button type="button" onClick={() => setSendButtonPanelOpen(!sendButtonPanelOpen)} className={`absolute right-full top-1/2 -translate-y-1/2 mr-1 p-1.5 text-neutral-400 hover:text-neutral-600 hover:bg-neutral-50 rounded-lg transition-all duration-300 ${sendButtonPanelOpen || isBtnHovered
                    ? "opacity-100 translate-x-0 pointer-events-auto"
                    : "opacity-0 -translate-x-2 pointer-events-none"}`} title="Configurar botão">
                          <Settings className="w-5 h-5"/>
                        </button>
                        <button type="button" onClick={() => {
                    setSendButtonPanelOpen(!sendButtonPanelOpen);
                }} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold shadow-lg hover:opacity-90 transition-all active:scale-95" style={{ backgroundColor: btnBgColor, color: btnTextColor }}>
                    ...
                    {sendButtonPanelOpen && (<div className="w-full max-w-sm p-4 bg-white rounded-xl border border-neutral-200 shadow-xl animate-in fade-in slide-in-from-top-2 duration-200 z-10">
                        ...
                        return (<button key={tab.key} type="button" onClick={() => setSendButtonPanelTab(tab.key)} className={`flex-1 px-2 py-1 text-xs font-medium rounded-md transition-all ${active
                        ...
                        {sendButtonPanelTab === "text" && (<div className="space-y-2">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Texto do botão</label>
                            <input type="text" value={btnText} autoFocus onChange={(e) => setBtnText(e.target.value)} placeholder="Digite o texto do botão..." className="w-full px-3 py-2 bg-neutral-50 border border-neutral-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#FF007F] transition-all" data-no-toolbar="true" onKeyDown={(e) => {
                        ...
                        {sendButtonPanelTab === "icon" && (<div className="space-y-2">
                            ...
                            return (<button key={key} type="button" title={label} onClick={() => setBtnIcon(key)} className={`flex items-center justify-center h-8 w-full rounded-lg border transition-all ${selected
                        ...
                        {sendButtonPanelTab === "position" && (<div className="space-y-2">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Posição do ícone</label>
                            <div className="grid grid-cols-2 gap-1.5">
                              <button type="button" onClick={() => setBtnIconPosition("start")} className={`flex items-center justify-center gap-1.5 h-8 rounded-lg border text-xs font-medium transition-all ${btnIconPosition === "start"
                            ? "border-[#FF007F] bg-pink-50 text-[#FF007F]"
                            : "border-neutral-200 bg-white text-neutral-600 hover:border-neutral-300 hover:bg-neutral-50"}`}>
                                Início
                              </button>
                              <button type="button" onClick={() => setBtnIconPosition("end")} className={`flex items-center justify-center gap-1.5 h-8 rounded-lg border text-xs font-medium transition-all ${btnIconPosition === "end"
                            ? "border-[#FF007F] bg-pink-50 text-[#FF007F]"
                            : "border-neutral-200 bg-white text-neutral-600 hover:border-neutral-300 hover:bg-neutral-50"}`}>
                                Fim
                              </button>
               ...
               {slashMenuOpen && !slashReplaceForBlockId && filteredOptions.length > 0 && (() => {
                ...
                return (<div className="fixed z-50 w-64 max-h-80 overflow-y-auto bg-white rounded-lg border border-neutral-200 shadow-lg py-1" style={{ top: slashMenuPos.top, left: slashMenuPos.left }}>
                  {(() => {
                        let runningIdx = 0;
                        return filteredSections.map((section) => (<div key={section.title}>
                        <p className="px-3 py-1.5 text-xs font-medium text-neutral-500">{section.title}</p>
                        {section.options.map((opt) => {
                                ...
                                return (<button key={opt.label} type="button" disabled={isDisabled} onMouseDown={(e) => {
                                        e.preventDefault();
                                        if (!isDisabled)
                                            insertOption(opt.label);
                                    }} onMouseEnter={() => !isDisabled && setSlashIndex(idx)} className={`w-full flex items-center gap-2.5 px-3 py-1.5 text-sm text-left transition-all ${idx === slashIndex ? "bg-neutral-100 text-neutral-900" : "text-neutral-700 hover:bg-neutral-50"} ${isDisabled ? "opacity-40 grayscale-[0.5] cursor-not-allowed bg-neutral-50/50" : ""}`}>
      ...
      <button type="button" className="fixed bottom-4 right-4 w-8 h-8 rounded-full border border-neutral-200 bg-white text-neutral-500 hover:text-neutral-900 flex items-center justify-center shadow-sm">
        <HelpCircle className="w-4 h-4"/>
      </button>


      {pickerOpen && (<div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/60 backdrop-blur-sm px-4 pt-24" onClick={() => { setPickerOpen(false); setPickerQuery(""); setPickerForBlockId(null); setPickerReplaceMode(false); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[70vh]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2 px-4 py-3 border-b border-neutral-100">
              <Search className="w-4 h-4 text-neutral-400"/>
              <input type="text" autoFocus value={pickerQuery} onChange={(e) => setPickerQuery(e.target.value)} placeholder="Buscar perguntas, campos e blocos de layout..." className="flex-1 bg-transparent outline-none text-sm text-neutral-800 placeholder:text-neutral-400" onKeyDown={(e) => {
                if (e.key === "Escape") {
                    setPickerOpen(false);
                    setPickerQuery("");
                    setPickerForBlockId(null);
              ...
              <button type="button" onClick={() => { setPickerOpen(false); setPickerQuery(""); setPickerForBlockId(null); setPickerReplaceMode(false); }} className="text-neutral-400 hover:text-neutral-700" aria-label="Fechar">
                <X className="w-4 h-4"/>
              </button>
            </div>
            <div className="overflow-y-auto py-2">
              {pickerFilteredSections.length === 0 ? (<div className="px-6 py-12 text-center text-sm text-neutral-400">
                  Nenhuma opção encontrada
                </div>) : (pickerFilteredSections.map((section) => (<div key={section.title} className="mb-2">
                    <p className="px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-neutral-400">
                      {section.title}
                    </p>
                    {section.options.map((opt) => {
                    const Icon = opt.icon;
                    return (<button key={opt.label} type="button" onClick={() => insertBlockFromPicker(opt.label)} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-left text-neutral-700 hover:bg-neutral-100 transition-colors">
      ...
      {isHistoryOpen && (<div className="fixed inset-0 z-50 flex">
          ...
          <aside className="relative ml-auto w-[320px] h-full bg-white shadow-xl flex flex-col animate-in slide-in-from-right duration-200">
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200">
              ...
              <div className="flex items-center gap-2 text-neutral-400">
                ...
                <button type="button" onClick={closeHistory} className="hover:text-neutral-700" aria-label="Fechar">
                  <X className="w-4 h-4"/>
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              <button type="button" onClick={() => previewVersion("current")} className={`w-full text-left px-4 py-3 border-b border-neutral-100 ${selectedVersion === "current" ? "bg-neutral-50" : "hover:bg-neutral-50"}`}>
              ...
              <ul>
                {versionHistory.filter((v) => !v.isCurrent).map((v) => (<li key={v.id}>
                    <button type="button" onClick={() => previewVersion(v.id)} className={`w-full text-left px-4 py-3 ${selectedVersion === v.id ? "bg-neutral-100" : "hover:bg-neutral-50"}`}>
            ...
            <div className="p-3 border-t border-neutral-200">
              <button type="button" onClick={restoreSelected} disabled={selectedVersion === "current"} className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-medium px-4 py-2 rounded-md transition-colors">
                Restaurar
              </button>
            </div>
          </aside>
      ...
      {isSettingsOpen && (<div className="fixed inset-0 z-50 bg-white overflow-y-auto">
          <div className="sticky top-0 bg-white border-b border-neutral-100 px-6 py-3 flex items-center">
            <button type="button" onClick={() => setIsSettingsOpen(false)} className="inline-flex items-center gap-1 text-sm text-neutral-700 hover:text-neutral-900 border border-neutral-200 rounded-md px-2 py-1">
          ...
          <div className="max-w-3xl mx-auto px-8 pt-10 pb-20">
            ...
            <div className="flex items-center gap-1 bg-neutral-100 p-1 rounded-lg w-fit mb-10">
              <button onClick={() => setSettingsActiveTab("geral")} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${settingsActiveTab === "geral" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}>
                Geral
              </button>
              <button onClick={() => setSettingsActiveTab("compartilhar")} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${settingsActiveTab === "compartilhar" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}>
                Compartilhar
              </button>
              <button onClick={() => setSettingsActiveTab("envios")} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${settingsActiveTab === "envios" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}>
                Envios
              </button>
              <button onClick={() => setSettingsActiveTab("insights")} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${settingsActiveTab === "insights" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}>
                Insights
              </button>
              <button onClick={() => setSettingsActiveTab("emails")} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${settingsActiveTab === "emails" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}>
                E-mails
              </button>
              <button onClick={() => setSettingsActiveTab("apresentacao")} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${settingsActiveTab === "apresentacao" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}>
                Apresentação
              </button>
            </div>


            {settingsActiveTab === "geral" && (<section className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <h2 className="text-sm font-semibold text-neutral-900">Configurações gerais</h2>
                <div className="mt-4 border-t border-neutral-200">
                <SettingsRow title="Redirecionar ao concluir" description="Redirecionar para uma URL personalizada quando o checklist for enviado." control={<Switch checked={redirectOnCompletion} onCheckedChange={setRedirectOnCompletion}/>}/>
                {redirectOnCompletion && (<div className="pl-6 pb-4 animate-in fade-in slide-in-from-top-1 duration-200">
                    <input type="url" value={redirectUrl} onChange={(e) => setRedirectUrl(e.target.value)} placeholder="https://exemplo.com/obrigado" className="w-full max-w-md text-sm border border-neutral-300 rounded-md px-3 py-2 bg-white outline-none focus:border-pink-500"/>
                    <p className="text-xs text-neutral-500 mt-1.5">O respondente será enviado para esta URL após clicar em enviar.</p>
                  </div>)}
                <SettingsRow title="Barra de progresso" description="A barra de progresso oferece uma forma clara para os respondentes entenderem o quanto do checklist eles já completaram, incentivando-os a continuar até o fim." control={<Switch checked={progressBar} onCheckedChange={setProgressBar}/>}/>
                <SettingsRow title="Envios parciais" badge={{ label: "Pro", color: "bg-pink-100 text-pink-600" }} description="Colete respostas de pessoas que preencheram parte do seu checklist, mas não clicaram no botão de enviar. Você não pode exportar envios parciais com integrações, nem habilitar notificações por email." control={<Switch checked={partialSubmissions} onCheckedChange={setPartialSubmissions}/>}/>
                <SettingsRow title="Marca ChecklistApp" badge={{ label: "Pro", color: "bg-pink-100 text-pink-600" }} description='Mostrar "Feito com ChecklistApp" no seu checklist.' control={<Switch checked={checklistBranding} onCheckedChange={setChecklistBranding}/>}/>
                <SettingsRow title="Retenção de dados de envios" badge={{ label: "Pro", color: "bg-pink-100 text-pink-600" }} description="Exclua automaticamente as respostas (envios) após um período definido. O seu checklist original nunca será excluído, apenas as respostas recebidas serão apagadas para conformidade." control={<Switch checked={dataRetention} onCheckedChange={async (checked) => {
                ...
                {dataRetention && (<div className="pl-6 pb-4 animate-in fade-in slide-in-from-top-1 duration-200">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-medium text-neutral-500">Excluir após</span>
                      <select value={retentionDays} onChange={async (e) => {
              ...
              <div className="mt-4 border-t border-neutral-200">
                <SettingsRow title="Notificações por email para você" description="Receba um email a cada novo envio de formulário." control={<Switch checked={selfEmailNotif} onCheckedChange={setSelfEmailNotif}/>}/>
                <SettingsRow title="Notificações por email para respondentes" badge={{ label: "Pro", color: "bg-pink-100 text-pink-600" }} description="Envie um email com texto personalizado aos respondentes após o envio do formulário." control={<button onClick={() => setSettingsActiveTab("emails")} className="text-xs font-bold text-pink-600 hover:text-pink-700">
                      Configurar
                    </button>}/>
              ...
              <div className="mt-4 border-t border-neutral-200">
                <SettingsRow title="Proteger formulário com senha" description="Habilite esta configuração para exigir uma senha antes que os respondentes possam acessar o formulário." control={<Switch checked={passwordProtect} onCheckedChange={setPasswordProtect}/>}/>
                {passwordProtect && (<div className="pl-6 pb-4 animate-in fade-in slide-in-from-top-1 duration-200">
                    <input type="text" value={formPassword} onChange={(e) => setFormPassword(e.target.value.slice(0, 100))} placeholder="Digite a senha" className="text-xs border border-neutral-300 rounded-md px-2 py-1.5 bg-white outline-none focus:border-pink-500 w-64"/>
                  </div>)}
                <SettingsRow title="Fechar formulário" description="As pessoas não poderão mais responder a este formulário." control={<Switch checked={closeForm} onCheckedChange={setCloseForm}/>}/>
                <SettingsRow title="Fechar formulário em uma data agendada" description="Agende uma data em que o formulário será fechado para novos envios." control={<Switch checked={closeFormScheduled} onCheckedChange={setCloseFormScheduled}/>}/>
                {closeFormScheduled && (<div className="pl-6 pb-4 animate-in fade-in slide-in-from-top-1 duration-200">
                    <input type="datetime-local" value={closeFormDate} onChange={(e) => setCloseFormDate(e.target.value)} className="text-xs border border-neutral-300 rounded-md px-2 py-1.5 bg-white outline-none focus:border-pink-500"/>
                  </div>)}
                <SettingsRow title="Limitar número de envios" description="Defina quantos envios você quer receber no total." control={<Switch checked={limitSubmissions} onCheckedChange={setLimitSubmissions}/>}/>
                {limitSubmissions && (<div className="pl-6 pb-4 animate-in fade-in slide-in-from-top-1 duration-200 flex items-center gap-2">
                    <span className="text-xs font-medium text-neutral-500">Fechar após</span>
                    <input type="number" min={1} value={submissionLimit} onChange={(e) => setSubmissionLimit(Math.max(1, Number(e.target.value) || 1))} className="text-xs font-bold border border-neutral-300 rounded-md px-2 py-1.5 bg-white outline-none focus:border-pink-500 w-24"/>
                    <span className="text-xs font-medium text-neutral-500">envios</span>
                  </div>)}
                <SettingsRow title="Mensagem de formulário fechado" description="Isso é o que os destinatários verão se você fechar o formulário com uma das opções acima." control={<Switch checked={closedFormMessage} onCheckedChange={setClosedFormMessage}/>}/>
                {closedFormMessage && (<div className="pl-6 pb-4 animate-in fade-in slide-in-from-top-1 duration-200">
                    <textarea value={closedMessageText} onChange={(e) => setClosedMessageText(e.target.value.slice(0, 500))} rows={3} placeholder="Este formulário está fechado e não está mais aceitando respostas." className="w-full max-w-md text-xs border border-neutral-300 rounded-md px-2 py-1.5 bg-white outline-none focus:border-pink-500 resize-y"/>
                  </div>)}
              </div>

              <div className="mt-8 flex items-center justify-center gap-6">
                <button type="button" className="text-sm text-neutral-400 cursor-not-allowed border border-neutral-200 rounded-md px-3 py-1.5" disabled>
                  Salvar alterações
                </button>
            ...
            {settingsActiveTab === "compartilhar" && (<section className="animate-in fade-in slide-in-from-bottom-2 duration-300 py-10">
                {checklistId ? (<div className="space-y-6">
                    <div>
                    ...
                    <div className="p-4 bg-neutral-50 border border-neutral-200 rounded-xl space-y-4 shadow-sm">
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          <Link to="/dominios" className="p-2 text-neutral-400 hover:text-pink-500 hover:bg-pink-50 rounded-lg transition-all" title="Gerenciar domínios">
                            <Settings className="w-4 h-4"/>
                          </Link>
                          <button disabled={!publicShareId} onClick={() => {
                        if (!publicShareId) {
                    ...
                    <div className="flex gap-4">
                      <a href={customDomain && customDomainStatus === 'verified' ? `https://${customDomain}/${publicShareId}` : `/c/${publicShareId}`} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 p-4 border border-neutral-200 rounded-xl text-sm font-bold text-neutral-700 hover:bg-neutral-50 transition-colors">
                        <ExternalLink className="w-4 h-4"/> Abrir checklist
                      </a>
                    </div>

                  </div>) : (<div className="bg-neutral-50 rounded-2xl p-12 border border-dashed border-neutral-200 text-center">
            ...
            {settingsActiveTab === "envios" && (<section className="animate-in fade-in slide-in-from-bottom-2 duration-300 py-10">
                {settingsChecklistId ? (<Suspense fallback={<div className="text-center py-12 text-neutral-500">Carregando envios…</div>}>
                    <SubmissionsTab checklistId={settingsChecklistId} onRetentionChange={(enabled, days) => {
                        setDataRetention(enabled);
                        setRetentionDays(days);
                    }}/>
                  </Suspense>) : (<div className="bg-neutral-50 rounded-2xl p-12 border border-dashed border-neutral-200 text-center">
            ...
            {settingsActiveTab === "insights" && (<section className="animate-in fade-in slide-in-from-bottom-2 duration-300 py-10">
                {settingsChecklistId ? (<Suspense fallback={<div className="text-center py-12 text-neutral-500">Carregando insights…</div>}>
            ...
            {settingsActiveTab === "emails" && (<section className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="mt-4 border-t border-neutral-200">
                  <div className="py-6 border-b border-neutral-200">
                    ...
                    <SettingsRow title="Receber notificações de novos envios" description="Você receberá um e-mail toda vez que alguém responder o formulário." control={<Switch checked={selfEmailNotif} onCheckedChange={setSelfEmailNotif}/>}/>
                    {selfEmailNotif && (<div className="mt-4 space-y-4 pl-4 border-l-2 border-neutral-100 animate-in fade-in slide-in-from-left-1">
                        <CustomField label="Seu e-mail de destino">
                          <input type="email" value={ownerEmailAddress || user?.email || ""} onChange={(e) => setOwnerEmailAddress(e.target.value)} placeholder="seu@email.com" className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2 outline-none focus:border-neutral-400"/>
                        </CustomField>
                      </div>)}
                  </div>
                  ...
                  <div className="py-6 border-b border-neutral-200">
                    ...
                    <CustomField label="Selecione o domínio verificado">
                      <select value={customEmailDomainId || ""} onChange={(e) => setCustomEmailDomainId(e.target.value || null)} className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2 bg-white">
                        <option value="">Padrão (tieck.com)</option>
                        {userDomains.filter(d => d.status === 'verified').map(d => (<option key={d.id} value={d.id}>{d.domain}</option>))}
                      </select>
                    </CustomField>
                  ...
                  <div className="py-6 border-b border-neutral-200">
                    ...
                    <SettingsRow title="Enviar e-mail automático de confirmação" description="A pessoa que respondeu recebe um e-mail automático de confirmação após o envio." control={<Switch checked={respondentEmailNotif} onCheckedChange={setRespondentEmailNotif}/>}/>

                    {respondentEmailNotif && (<div className="mt-4 space-y-6 pl-4 border-l-2 border-neutral-100 animate-in fade-in slide-in-from-left-1">
                        <CustomField label="Qual campo contém o e-mail do respondente?">
                          <select value={respondentEmailFieldId} onChange={(e) => setRespondentEmailFieldId(e.target.value)} className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2 bg-white">
                            <option value="">Selecione um campo</option>
                            {blocks.filter(b => b.type === 'short-answer' || b.type === 'email').map(b => (<option key={b.id} value={b.id}>{b.subtitle || b.placeholder || `Campo ${b.id.slice(0, 4)}`}</option>))}
                          </select>
                        </CustomField>

                        <CustomField label="Assunto do e-mail">
                          <input type="text" value={respondentEmailSubject} onChange={(e) => setRespondentEmailSubject(e.target.value)} className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2"/>
                        </CustomField>

                        <CustomField label="Mensagem personalizada">
                          <textarea value={respondentEmailMessage} onChange={(e) => setRespondentEmailMessage(e.target.value)} rows={4} className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2 resize-none" placeholder="Dica: use {{title}} para inserir o nome do checklist"/>
                        </CustomField>

                        <SettingsRow title="Incluir cópia das respostas" description="O e-mail conterá um resumo de todas as perguntas e respostas enviadas." control={<Switch checked={includeResponsesInEmail} onCheckedChange={setIncludeResponsesInEmail}/>}/>
                      </div>)}
                  </div>
                </div>

                <div className="mt-8 flex items-center justify-center gap-6">
                  <button type="button" onClick={() => saveChecklist()} className="text-sm bg-[#FF007F] text-white font-bold rounded-md px-6 py-2 hover:opacity-90 transition-opacity">
                    Salvar configurações
                  </button>
                </div>
              </section>)}

            {settingsActiveTab === "apresentacao" && (<section className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="mt-6 space-y-6">
                  <CustomField label="Título">
                    <input type="text" value={thankYouTitle} onChange={(e) => setThankYouTitle(e.target.value)} placeholder="Obrigado por preencher este formulário!" className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2 outline-none focus:border-neutral-400"/>
                  </CustomField>
                  <CustomField label="Descrição">
                    <textarea value={thankYouDescription} onChange={(e) => setThankYouDescription(e.target.value)} rows={3} placeholder="Feito com Tieck, a forma mais simples de criar checklists gratuitamente." className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2 outline-none focus:border-neutral-400 resize-none"/>
                  </CustomField>
                </div>
      ...
      {isCustomizeOpen && (<aside className="fixed top-0 right-0 z-40 w-[360px] h-full bg-white border-l border-neutral-200 shadow-xl flex flex-col animate-in slide-in-from-right duration-200">
          <div className="flex items-center justify-between px-5 py-3 border-b border-neutral-200">
            <h2 className="text-base font-semibold text-neutral-900">Personalizar</h2>
            <button type="button" onClick={() => setIsCustomizeOpen(false)} className="text-neutral-400 hover:text-neutral-700" aria-label="Fechar">
              <X className="w-4 h-4"/>
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
            ...
            <div className="relative">
              {user?.plan_type !== 'pro' && (<div className="absolute inset-x-[-20px] inset-y-[-10px] z-50 bg-white/60 backdrop-blur-[1px] flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <button onClick={() => navigate({ to: '/membros' })} className="px-4 py-1.5 bg-[#FF007F] text-white rounded-lg text-xs font-bold hover:opacity-90 transition-opacity shadow-sm">
                    Fazer upgrade
                  </button>
                </div>)}
    ...
    {selectionToolbar.visible && (() => {
            ...
            return (<div className="fixed z-[200] flex items-center gap-0.5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-lg px-1.5 py-1" style={{ top: selectionToolbar.top, left: selectionToolbar.left }} onMouseDown={(e) => e.preventDefault()} data-color-picker-root>
          <button type="button" onClick={() => handleFormat("bold")} title="Negrito" className={`p-1.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors ${isBold ? "text-[#FF007F]" : "text-neutral-700 dark:text-neutral-200"}`}>
            <Bold className="w-4 h-4" strokeWidth={2.5}/>
          </button>
          <button type="button" onClick={() => handleFormat("italic")} title="Itálico" className={`p-1.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors ${isItalic ? "text-[#FF007F]" : "text-neutral-700 dark:text-neutral-200"}`}>
            <Italic className="w-4 h-4" strokeWidth={2.5}/>
          </button>
          <button type="button" onClick={() => handleFormat("underline")} title="Sublinhado" className={`p-1.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors ${isUnderline ? "text-[#FF007F]" : "text-neutral-700 dark:text-neutral-200"}`}>
          ...
          <div className="relative">
            <button type="button" onClick={() => {
                ...
                }} title="Cor do texto" className={`p-1.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors flex items-center ${colorPickerOpen ? "text-[#FF007F]" : "text-neutral-700 dark:text-neutral-200"}`}>
            ...
            {colorPickerOpen && (<div className="absolute top-full left-0 mt-2 z-[210]" data-color-picker-root>
                <ColorPicker value={currentTextColor} onChange={(c) => {
          ...
          <div className="relative">
            <button type="button" onClick={() => {
                ...
                }} title="Cor de destaque" className={`p-1.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors flex items-center ${highlightPickerOpen ? "text-[#FF007F]" : "text-neutral-700 dark:text-neutral-200"}`}>
              <div className="flex flex-col items-center">
            ...
            {highlightPickerOpen && (<div className="absolute top-full left-0 mt-2 z-[210]" data-color-picker-root>
                <ColorPicker value={currentHighlightColor} onChange={(c) => {
          ...
          <div className="relative" data-link-popover-root>
            <button type="button" onMouseDown={(e) => {
                ...
                }} title="Adicionar link" className={`p-1.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors ${linkPopoverOpen ? "text-[#FF007F]" : "text-neutral-700 dark:text-neutral-200"}`}>
              <Link2 className="w-4 h-4" strokeWidth={2.5}/>
            </button>
            {linkPopoverOpen && (<div className="absolute top-full left-0 mt-2 z-[210] bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-2xl p-1.5 flex items-center gap-1.5" onMouseDown={(e) => e.stopPropagation()}>
                <input type="url" autoFocus value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} onKeyDown={(e) => {
                        if (e.key === "Enter") {
                            e.preventDefault();
                    ...
                    }} placeholder="Cole qualquer link da web" className="text-sm px-2.5 py-1.5 rounded-md border border-blue-300 outline-none focus:border-blue-500 bg-white dark:bg-neutral-800 dark:border-neutral-700 w-[220px]"/>
                <button type="button" onClick={() => {
                        const url = linkUrl.trim();
                        if (!url)
                            return;
      ...
      <AlertDialog open={publishBlocker !== null} onOpenChange={(open) => { if (!open)
...
function CustomField({ label, description, children }) {
    return (<div className="space-y-1.5">
      <div className="flex flex-col">
...
function CustomColor({ label, value, onChange, hint, }) {
    return (<div>
      ...
      <div className="flex items-center gap-2 border border-neutral-300 rounded-md px-2 py-1.5">
        ...
        <input type="text" value={value} onChange={(e) => onChange(e.target.value)} className="flex-1 text-sm bg-transparent outline-none min-w-0"/>
      </div>
    </div>);
}
function SettingsRow({ title, description, control, badge, }) {
    return (<div className="grid grid-cols-[1fr_auto] gap-6 py-5 border-b border-neutral-200 items-start">
      <div>
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-neutral-900">{title}</h3>
          {badge && (<span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${badge.color}`}>
              {badge.label}
            </span>)}
        </div>
...
function ImageBlock({ src, variant, onChange, onRemove, position, onPositionChange, logoSettings, }) {
    ...
    if (src) {
        ...
        return (<div className={`relative ${isProfile ? "group flex items-center justify-center px-12" : "group"}`}>
        {isProfile && !isEditingPosition && (<button type="button" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => {
                    e.stopPropagation();
        ...
        <div className={`relative ${containerClass} ${isEditingPosition ? "ring-2 ring-pink-500 cursor-move" : ""}`} style={containerStyle} onMouseDown={(e) => {
                if (!isEditingPosition || !imgRef.current)
                    return;
                e.preventDefault();
          ...
          {isEditingPosition && (<div className="absolute inset-0 z-20 pointer-events-none">
              {/* Vertical Centering Guide */}
              <div className={`absolute top-0 bottom-0 left-1/2 w-0.5 bg-pink-500 shadow-[0_0_8px_rgba(236,72,153,0.5)] transition-opacity duration-150 ${isMoving && Math.abs(posX) < 1 ? "opacity-100" : "opacity-0"}`}/>
              {/* Horizontal Centering Guide */}
              <div className={`absolute top-1/2 left-0 right-0 h-0.5 bg-pink-500 shadow-[0_0_8px_rgba(236,72,153,0.5)] transition-opacity duration-150 ${isMoving && Math.abs(posY) < 1 ? "opacity-100" : "opacity-0"}`}/>
              
              <div className="absolute inset-0 flex flex-col items-center justify-end pb-4 pointer-events-auto">
                <div data-controls className="relative z-30 bg-white rounded-full shadow-lg px-4 py-2 flex items-center gap-4 animate-in fade-in slide-in-from-bottom-2" onMouseDown={(e) => e.stopPropagation()}>
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => {
                    const currentZoom = position?.zoom || 1;
                    onPositionChange?.({ ...(position || { x: 0, y: 0, zoom: 1 }), zoom: Math.max(1, currentZoom - 0.1) });
                  ...
                  <input type="range" min="1" max="10" step="0.01" value={position?.zoom || 1} onChange={(e) => {
          ...
          {!isEditingPosition && (<>
              <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 z-20 bg-black/40 ${isProfile ? "cursor-pointer" : ""}`} onClick={() => isProfile && fileInputRef.current?.click()}>
                {!isProfile && (<>
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2 bg-white rounded-full text-black hover:bg-neutral-50 shadow-sm transition-transform hover:scale-110" title="Trocar imagem">
                      <Upload className="w-4 h-4"/>
                    </button>
                    
                    <button type="button" onClick={() => setIsEditingPosition(true)} className="p-2 bg-white rounded-full text-black hover:bg-neutral-50 shadow-sm transition-transform hover:scale-110" title="Ajustar posição">
                      <Move className="w-4 h-4"/>
                    </button>
                  </>)}
              </div>

              {variant !== "profile" && (<button type="button" onClick={(e) => {
                        e.stopPropagation();
                        onRemove?.();
                    }} className="absolute top-2 right-2 p-1.5 bg-white/90 border border-neutral-200 rounded-full shadow-sm text-neutral-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity z-30" title="Remover">
    ...
    if (variant === "profile" || variant === "cover") {
        const isProfile = variant === "profile";
        return (<button type="button" onClick={() => fileInputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => {
                e.preventDefault();
                ...
                : "flex items-center justify-center w-full h-64 bg-neutral-100 hover:bg-neutral-200/70 transition-colors text-neutral-300"} aria-label={isProfile ? "Adicionar logo" : "Adicionar capa"}>
    ...
    if (!expanded) {
        return (<CollapsedMediaPlaceholder icon={ImageIcon} label="Adicionar uma imagem" onClick={() => setExpanded(true)}/>);
    }
    return (<div ref={wrapperRef} className={`${variant === 'profile' ? 'w-64' : 'w-full'} rounded-md border border-neutral-200 bg-white`}>
      <div className="flex items-center gap-4 px-4 pt-3 text-sm border-b border-neutral-100">
        {["upload", "link", "unsplash"].map((t) => (<button key={t} type="button" onClick={() => setTab(t)} className={`pb-2 -mb-px capitalize ${tab === t
      ...
      <div className="p-4">
        {tab === "upload" && (<button type="button" onClick={() => fileInputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => {
                e.preventDefault();
                const f = e.dataTransfer.files?.[0];
                if (f)
                    handleFile(f);
        ...
        {tab === "link" && (<div className="flex flex-col gap-2 py-6">
            <input type="url" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="Cole a URL da imagem" className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-md outline-none focus:border-neutral-400"/>
            <button type="button" disabled={!linkUrl.trim()} onClick={() => onChange(linkUrl.trim())} className="self-start px-3 py-1.5 text-sm bg-neutral-900 text-white rounded-md disabled:opacity-40">
              Inserir imagem
            </button>
          </div>)}
        {tab === "unsplash" && (<div className="py-12 text-center text-sm text-neutral-400">
            Busca do Unsplash em breve.
          </div>)}
      </div>
    </div>);
}
function VideoBlock({ src, onChange, onRemove, }) {
    const [linkUrl, setLinkUrl] = useState("");
    const [expanded, setExpanded] = useState(true);
    const wrapperRef = useRef(null);
    useCollapseOnOutside(wrapperRef, expanded, () => setExpanded(false));
    const toEmbedUrl = (raw) => {
        ...
        if (yt)
            return { type: "iframe", url: `https://www.youtube.com/embed/${yt[1]}` };
        // Vimeo
        const vm = url.match(/vimeo\.com\/(?:video\/)?(\d+)/);
        if (vm)
            return { type: "iframe", url: `https://player.vimeo.com/video/${vm[1]}` };
        // Loom
        const lm = url.match(/loom\.com\/(?:share|embed)\/([a-zA-Z0-9]+)/);
        if (lm)
            return { type: "iframe", url: `https://www.loom.com/embed/${lm[1]}` };
        // mp4 / webm / ogg
    ...
    if (src) {
        ...
        <div className="w-full aspect-video rounded-md border border-neutral-200 overflow-hidden bg-black">
          {embed.type === "iframe" ? (<iframe src={embed.url} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>) : (<video src={embed.url} controls className="w-full h-full"/>)}
        </div>
        <button type="button" onClick={onRemove} className="absolute top-2 right-2 px-2 py-1 text-xs bg-white/90 border border-neutral-200 rounded shadow-sm text-neutral-600 hover:text-neutral-900 opacity-0 group-hover:opacity-100 transition-opacity">
          Remover
        </button>
      </div>);
    }
    if (!expanded) {
        return (<CollapsedMediaPlaceholder icon={Video} label="Incorporar um vídeo" onClick={() => setExpanded(true)}/>);
    }
    return (<div ref={wrapperRef} className="w-full rounded-md border border-neutral-200 bg-white">
      <div className="flex items-center gap-4 px-4 pt-3 text-sm border-b border-neutral-100">
        <button type="button" className="pb-2 -mb-px text-neutral-900 border-b-2 border-neutral-900 font-medium">
          Embed link
        </button>
      </div>
      <div className="p-4 flex flex-col gap-3">
        <input type="url" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="Cole o link do vídeo" className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-md outline-none focus:border-neutral-400"/>
        <button type="button" disabled={!linkUrl.trim()} onClick={() => onChange(linkUrl.trim())} className="w-full px-3 py-2 text-sm bg-neutral-900 text-white rounded-md disabled:opacity-40 font-medium">
          Inserir vídeo
        </button>
...
function useCollapseOnOutside(ref, active, onOutside) {
    useEffect(() => {
        if (!active)
            return;
        const handler = (e) => {
...
function EmbedBlock({ src, onChange, onRemove, }) {
    const [linkUrl, setLinkUrl] = useState("");
    const [expanded, setExpanded] = useState(true);
    const wrapperRef = useRef(null);
    useCollapseOnOutside(wrapperRef, expanded, () => setExpanded(false));
    if (src) {
        return (<div className="group relative w-full">
        <iframe src={src} className="w-full h-[480px] rounded-md border border-neutral-200 bg-white" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowFullScreen/>
        <button type="button" onClick={onRemove} className="absolute top-2 right-2 px-2 py-1 text-xs bg-white/90 border border-neutral-200 rounded shadow-sm text-neutral-600 hover:text-neutral-900 opacity-0 group-hover:opacity-100 transition-opacity">
          Remover
        </button>
      </div>);
    ...
    if (!expanded) {
        return (<CollapsedMediaPlaceholder icon={Code} label="Incorporar qualquer coisa" onClick={() => setExpanded(true)}/>);
    }
    return (<div ref={wrapperRef} className="w-full rounded-md border border-neutral-200 bg-white">
      <div className="flex items-center gap-4 px-4 pt-3 text-sm border-b border-neutral-100">
        <button type="button" className="pb-2 -mb-px text-neutral-900 border-b-2 border-neutral-900 font-medium">
      ...
      <div className="p-4 flex flex-col gap-3">
        <input type="url" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="Cole o link" className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-md outline-none focus:border-neutral-400"/>
        <button type="button" disabled={!linkUrl.trim()} onClick={() => onChange(linkUrl.trim())} className="w-full px-3 py-2 text-sm bg-neutral-900 text-white rounded-md disabled:opacity-40 font-medium">
          Incorporar link
        </button>
        <p className="text-xs text-neutral-400 text-center">
...
function CollapsedMediaPlaceholder({ icon: Icon, label, onClick, }) {
    return (<button type="button" onClick={onClick} className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-neutral-500 bg-white border border-neutral-200 rounded-md hover:bg-neutral-50 hover:text-neutral-700 transition-colors text-left">
...
function RatingStars({ value, onChange }) {
    ...
    return (<div className="group w-full flex items-center gap-1" onMouseLeave={() => setHover(0)}>
      {Array.from({ length: 5 }, (_, i) => {
            const n = i + 1;
            const filled = n <= display;
            return (<button key={n} type="button" onMouseEnter={() => setHover(n)} onClick={() => onChange(value === n ? 0 : n)} aria-label={`${n} estrela${n > 1 ? "s" : ""}`} className="p-1 transition-transform hover:scale-110">
            <Star className="w-8 h-8 transition-colors" style={{
...
function Item({ icon: Icon, label, onClick }) {
    return (<li className="flex items-center gap-2 hover:text-neutral-900 cursor-pointer" onClick={onClick}>
      <Icon className="w-4 h-4 text-neutral-400"/>
      <span>{label}</span>
    </li>);
}
function SignaturePad({ dataUrl, onChange }) {
    ...
    return (<div className="group w-full">
      <div className="relative w-full rounded-md border border-dashed border-neutral-300 bg-white">
        <button type="button" onClick={clear} className="absolute top-2 right-3 text-sm text-neutral-700 hover:text-neutral-900 z-10">
          Clear
        </button>
        <canvas ref={canvasRef} onPointerDown={start} onPointerMove={move} onPointerUp={end} onPointerLeave={end} className="block w-full h-40 cursor-crosshair touch-none"/>
