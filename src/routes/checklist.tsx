...
import { getChecklistRenderStyle, getChecklistContainerClass } from "@/lib/checklist-render-styles";
    ...
    <div
      ...
      onKeyDown={(e) => {
        if (e.key === "Backspace") {
          ...
          if (!isInput || (e.target as any).value === "") {
            e.preventDefault();
      ...
      <main className="max-w-full mx-auto pb-32 relative w-full overflow-x-hidden">
        <div className="flex flex-col items-center w-full">
          <div className="w-full relative">
            ...
            {/* Cover and Profile */}
            <ChecklistCoverProfile blocks={blocks} settings={settings} />
...
function ChecklistAuthGuard({ children }: { children: React.ReactNode }) {
...
export function NovoChecklistPage() {
  ...
  // 5E.0C.1: separate checklist management from workspace RBAC. RBAC is resolved
  ...
  // 5E.0C.1: workspace-scoped resources (RBAC, members, deadline assignments)
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      // 5E.0C.1: reset ownership metadata when switching checklists; keep the
      ...
      const fetchExisting = async () => {
        ...
        try {
          ...
          if (data && !error) {
            // 5E.0C.1: persist only the minimal ownership context needed to
