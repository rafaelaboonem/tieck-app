...
import { Loader2, Camera, CheckCircle2, XCircle, ChevronDown, ChevronUp } from "lucide-react";
...
export function OpenAILabTest({ standard }: { standard: any }) {
  ...
  const analyze = async () => {
    if (!candidate || analyzing || !ready) return;
    setAnalyzing(true);
    setResult(null);
    try {
    ...
    <div className="space-y-6 pt-6 border-t">
      ...
      <div className="flex flex-col items-center">
        <div className="w-full max-w-md aspect-video rounded-xl border-2 border-dashed bg-muted flex items-center justify-center overflow-hidden relative mb-4">
          {preview ? (
            <>
              ...
              <div className="absolute top-2 right-2"><Button size="sm" variant="secondary" onClick={() => fileRef.current?.click()}>Trocar</Button></div>
            </>
          ) : (
            <Button variant="ghost" onClick={() => fileRef.current?.click()} disabled={!ready}><Camera className="mr-2 h-4 w-4" /> Tirar foto ou galeria</Button>
          )}
        ...
        <Button 
          ...
          disabled={!candidate || analyzing || !ready} 
          onClick={analyze}
        >
          {analyzing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Verificando a condição solicitada...</> : 'Analisar foto'}
        </Button>
      ...
      {result && (
        <div className="space-y-4 animate-in fade-in duration-500">
          {result.server_decision === 'approved' ? (
            <div className="p-6 rounded-xl border bg-emerald-50 text-emerald-900 border-emerald-200">
              ...
              <p className="text-sm opacity-90">{result.observed_evidence || 'A foto corresponde às referências e atende à condição solicitada.'}</p>
            </div>
          ) : result.server_decision === 'rejected' ? (
            <div className="p-6 rounded-xl border bg-amber-50 text-amber-900 border-amber-200">
              ...
              <p className="text-sm font-bold mb-1">{result.blocking_reasons || 'Não foi possível confirmar os critérios.'}</p>
          ...
          <div className="border rounded-xl overflow-hidden">
            <button className="w-full flex items-center justify-between p-4 bg-muted/50 hover:bg-muted/80 transition-colors" onClick={() => setShowTech(!showTech)}>
            ...
            {showTech && (
              <div className="p-4 bg-muted/20 text-[11px] font-mono grid grid-cols-2 gap-y-2 gap-x-4">
                ...
                <div>Tokens In: {result.telemetry?.usage?.input_tokens}</div>
                <div>Tokens Out: {result.telemetry?.usage?.output_tokens}</div>
